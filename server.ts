...
async function startServer() {
  ...
  // CORS Middleware for external frontend communication (e.g. Vercel deployment)
  ...
  app.get('/api/mercadopago/status', async (req, res) => {
    try {
      ...
      console.error('[MercadoPago Status Error]:', err);
      res.status(500).json({ status: 'error', message: 'Error consultando estado de Mercado Pago' });
    }
  });
    ...
    try {
      ...
      if (!platformConfig.isConfigured) {
        ...
        if (isExplicitTestMode) {
          ...
          return res.redirect(`${targetBase}/?mp_status=connected#minegocio`);
        }
  ...
  app.get('/api/mercadopago/callback', async (req, res) => {
    try {
      ...
      res.redirect(`${targetBase}/?mp_status=connected#minegocio`);
    } catch (err: any) {
  ...
  app.get('/api/mercadopago/merchants', async (_req, res) => {
    try {
  ...
  app.post('/api/mercadopago/config', async (req, res) => {
    try {
      const {
        businessId = 'default',
        enabled,
