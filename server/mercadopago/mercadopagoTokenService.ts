...
class MercadoPagoTokenServiceManager {
  ...
  public async getValidAccessToken(businessId: string): Promise<string | null> {
    ...
    // Check persistent store (Firestore primary with memory/disk fallback)
    const connection = await persistenceService.getConnectionAsync(cleanId);
  ...
  public async refreshAccessToken(
    ...
    const cleanId = String(businessId || '').trim();
    const conn = connection || (await persistenceService.getConnectionAsync(cleanId)) || persistenceService.getConnection(cleanId);
    ...
    try {
      ...
      // Persist updated credentials to Firestore primary and memory/disk backup
  ...
  public async saveOAuthTokens(params: {
    ...
    const existing = (await persistenceService.getConnectionAsync(cleanId)) || persistenceService.getConnection(cleanId) || {
      businessId: cleanId,
  ...
  /**
   * Revokes or clears tokens upon disconnect asynchronously.
   */
  public async clearTokensAsync(businessId: string): Promise<void> {
    const cleanId = String(businessId || '').trim();
    const existing = (await persistenceService.getConnectionAsync(cleanId)) || persistenceService.getConnection(cleanId);
    if (existing) {
      await persistenceService.saveConnectionAsync({
  ...
  /**
   * Revokes or clears tokens upon disconnect synchronously.
   */
