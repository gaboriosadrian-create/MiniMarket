...
export function findApprovedPayment(order: MercadoPagoOrder): MercadoPagoPayment | undefined {
  ...
  const paymentsList = (Array.isArray(order.payments) && order.payments.length > 0)
    ? order.payments
    : (Array.isArray(order.transactions?.payments) ? order.transactions.payments : []);
  if (paymentsList.length === 0) return undefined;
  return paymentsList.find((p) => isPaymentApproved(p)) || paymentsList[0];
}
 ...
 * 
 * Rules:
 * - 'CONFIRMED': The order is 'processed'/'closed'/'paid' OR order.status_detail is 'accredited'
...
export function mapMercadoPagoOrderStatus(order: MercadoPagoOrder): MappedOrderStatus {
  ...
  const merchantOrderStatus = (order.order_status || '').toLowerCase().trim();
  ...
  if (paymentsList.length > 0) {
    const hasApprovedPayment = paymentsList.some((p) => isPaymentApproved(p));
    if (hasApprovedPayment) {
      return 'CONFIRMED';
    }

    const allRejectedOrCancelled = paymentsList.every((p) => isPaymentFailed(p));
    if (allRejectedOrCancelled && (orderStatus === 'closed' || orderStatus === 'failed' || orderStatus === 'cancelled')) {
      return 'FAILED';
    }

    const hasPendingPayment = paymentsList.some((p) => isPaymentPending(p));
  ...
  if (
    ...
    orderDetail === 'approved'
  ) {
    return 'CONFIRMED';
  }

  if (orderStatus === 'opened' || orderStatus === 'created' || orderStatus === 'ready_to_process' || merchantOrderStatus === 'payment_required') {
    return 'PENDING';
  }
