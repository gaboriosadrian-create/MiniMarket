...
export interface CreateRealOrderResult {
  success: boolean;
  status: 'CREATED' | 'DISABLED' | 'ERROR' | 'INVALID_INPUT' | 'TERMINAL_BUSY' | 'CONFIG_ERROR';
  message: string;
  external_reference?: string;
  total_amount?: number;
  autoConfirm?: boolean;
...
export async function createRealMercadoPagoOrder(
  ...
  // 1. Tenant configuration & credential resolution:
  let config: MercadoPagoConfig;
  if (customConfig) {
    // Isolated customConfig (used in specific test fixtures)
    config = customConfig;
  } else if (!businessId) {
    if (!isTestEnv) {
      return {
        ...
        message: 'businessId es obligatorio para crear órdenes de Mercado Pago.',
      };
    }
    // In test environment, fallback to default test tenant
    config = await tenantConfigStore.getActiveRuntimeConfigAsync('default');
  } else {
    // Asynchronously resolve tenant config from Firestore with single source of truth for token
    config = await tenantConfigStore.getActiveRuntimeConfigAsync(businessId);
  }

  // 2. Check if integration is enabled for this tenant
  if (!config.enabled) {
    return {
      success: false,
      status: 'DISABLED',
      message: 'La integración de Mercado Pago no está habilitada para este comercio.',
    };
  }
  ...
  if (!config.accessToken || config.accessToken.trim().length === 0) {
    return {
      success: false,
      status: 'ERROR',
      message: 'Access Token de Mercado Pago no configurado o no verificado para este comercio.',
    };
  }
  ...
  // Strictly enforce required merchant / POS IDs per tenant: NO silent POC fallback!
  const userId = config.userId;
  const externalPosId = config.externalPosId;
  const pointTerminalId = input.pointTerminalId || config.pointTerminalId;
  ...
  if (!isPoint) {
    if (!userId || String(userId).trim().length === 0) {
      return {
        success: false,
        status: 'CONFIG_ERROR',
        message: 'Falta la configuración obligatoria del Comercio (userId) para procesar cobros con QR.',
      };
    }
    if (!externalPosId || String(externalPosId).trim().length === 0) {
      return {
        success: false,
        status: 'CONFIG_ERROR',
        message: 'Falta la configuración obligatoria de la Caja (externalPosId) para procesar cobros con QR.',
      };
    }
  } else {
    if (!pointTerminalId || String(pointTerminalId).trim().length === 0) {
      return {
        success: false,
        status: 'CONFIG_ERROR',
        message: 'Falta la terminal Point asociada (pointTerminalId) para procesar cobros con Point Smart.',
      };
    }
  }
  ...
  if (isPoint) {
    ...
    // Official Mercado Pago Orders API for In-Store QR / POS in TEST mode (/v1/orders)
    endpoint = `${apiBaseUrl}/v1/orders`;
    method = 'POST';
    ...
    payload = {
      ...
      description: input.description || input.title || 'Venta MiniMarket',
      external_reference: cleanRef,
      config: {
        qr: {
    ...
    // In-Store Static QR Orders API (Production legacy or custom endpoint)
  ...
  try {
      ...
      try {
        ...
        const sanitizeObj = (obj: any): any => {
          ...
          for (const [k, v] of Object.entries(obj)) {
            ...
            } else if (typeof v === 'string' && (v.startsWith('TEST-') || v.startsWith('APP_USR-') || v.startsWith('Bearer '))) {
              clean[k] = '[REDACTED]';
            } else {
              clean[k] = v;
            }
        ...
        const sanitized = sanitizeObj(jsonErr);
        const parts: string[] = [];
        if (sanitized.message) parts.push(`message: ${sanitized.message}`);
        if (sanitized.error && sanitized.error !== sanitized.message) parts.push(`error: ${sanitized.error}`);
        if (sanitized.status) parts.push(`status: ${sanitized.status}`);
        if (sanitized.cause) {
          const causeStr = typeof sanitized.cause === 'string'
            ? sanitized.cause
            : JSON.stringify(sanitized.cause);
          parts.push(`cause: ${causeStr}`);
        }
        if (sanitized.errors) {
          parts.push(`errors: ${JSON.stringify(sanitized.errors)}`);
        }

        if (parts.length > 0) {
          parsedMsg = `HTTP ${response.status} | ${parts.join(' | ')}`;
        } else {
          parsedMsg = `HTTP ${response.status} | ${JSON.stringify(sanitized)}`;
        }
      } catch {
        const cleanRaw = errorText
          .replace(/Bearer\s+[A-Za-z0-9_\-\.]+/gi, 'Bearer [REDACTED]')
          .replace(/TEST-[A-Za-z0-9_\-]+/gi, '[REDACTED]')
          .replace(/APP_USR-[A-Za-z0-9_\-]+/gi, '[REDACTED]')
          .slice(0, 500);
        parsedMsg = `HTTP ${response.status}: ${cleanRaw}`;
      }
    }
    ...
    return {
      ...
      message: isPoint
        ...
        : 'No se pudo crear la orden de Mercado Pago.',
      details: parsedMsg,
    };
  } catch (err: any) {
    clearTimeout(timeoutId);
...
export async function cancelMercadoPagoOrder(
  ...
  try {
    ...
    const orderIdToCancel = existingRecord?.orderId || cleanId;

    if ((isPoint || isTestMode) && orderIdToCancel) {
      // Cancel order via Orders API: POST /v1/orders/{id}/cancel
      await fetch(`${apiBaseUrl}/v1/orders/${encodeURIComponent(orderIdToCancel)}/cancel`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${config.accessToken}`,
          'Content-Type': 'application/json',
          'X-Idempotency-Key': `cancel-${cleanId}-${Date.now()}`,
        },
      });
