...
export function validateMercadoPagoOrder(
  ...
  const currency = (order.currency || order.currency_id || '').toUpperCase().trim();
  ...
  const orderUserId = order.user_id || order.collector_id || order.collector?.id;
  ...
  const rawTotal = order.total_amount !== undefined && order.total_amount !== null
    ? order.total_amount
    : (order.total_paid_amount !== undefined && order.total_paid_amount !== null ? order.total_paid_amount : order.paid_amount);
  const orderTotal = Number(rawTotal);
  if (isNaN(orderTotal) || orderTotal < 0) {
    return {
      valid: false,
      reason: `Importe total de orden inválido: ${rawTotal}`,
      code: 'INVALID_AMOUNT',
    };
  }
