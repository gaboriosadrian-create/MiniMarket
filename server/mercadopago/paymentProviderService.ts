...
class PaymentProviderServiceManager {
  ...
  public async getSanitizedStatusAsync(businessId: string = 'default') {
    ...
    // Ensure tenant config is asynchronously hydrated from Firestore for cold starts
    await tenantConfigStore.getConfigAsync(cleanId);

    let provider = this.providers.get(cleanId);

    // Refresh from Firestore async if not yet cached in memory
    if (!provider) {
  ...
  public getSanitizedStatus(businessId: string = 'default') {
    ...
    const isEnabled = Boolean(tenantConfig.enabled);
    ...
    return {
      ...
      provider: 'mercadopago',
      accountInfo: {
        userId: (isConnected || isEnabled) ? maskUserId(provider?.userId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.userId : tenantConfig.productionConfig.userId)) : undefined,
        siteId: provider?.siteId || 'MLA',
        externalStoreId: (isConnected || isEnabled) ? (provider?.externalStoreId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.externalStoreId : tenantConfig.productionConfig.externalStoreId) || generateExternalStoreId(cleanId)) : undefined,
        storeId: (isConnected || isEnabled) ? (provider?.storeId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.storeId : tenantConfig.productionConfig.storeId) || 'Configurada') : undefined,
        externalPosId: (isConnected || isEnabled) ? (provider?.externalPosId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.externalPosId : tenantConfig.productionConfig.externalPosId) || generateExternalPosId(cleanId)) : undefined,
        posId: (isConnected || isEnabled) ? (provider?.posId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.posId : tenantConfig.productionConfig.posId) || 'Configurada') : undefined,
        connectedAt: provider?.connectedAt || tenantConfig.updatedAt,
        accountEmail: provider?.accountEmail,
        accountNickname: provider?.accountNickname || (isConnected ? 'Cuenta vinculada correctamente' : (isEnabled ? 'Configuración habilitada' : undefined)),
      },
