...
class PaymentProviderServiceManager {
  ...
  public async getSanitizedStatusAsync(businessId: string = 'default') {
    const cleanId = String(businessId || 'default').trim();
    const isDefault = cleanId === 'default';

    // Fetch persisted connection directly from Firestore as the single source of truth
    const persisted = await persistenceService.getConnectionAsync(cleanId);

    // If no persisted connection exists
    if (!persisted) {
      if (!isDefault) {
        return {
          enabled: false,
      ...
      return this.getSanitizedStatus(cleanId);
    }

    // If persisted status is DISCONNECTED, inactive or tokens are empty
    if (persisted.status !== 'CONNECTED' || !persisted.accessToken) {
      const cached = this.providers.get(cleanId);
      if (cached) {
    ...
    // Persisted state is strictly CONNECTED with active token
    const isConnected = true;
    const isEnabled = persisted.enabled !== undefined ? Boolean(persisted.enabled) : true;
    ...
    return {
      ...
      provider: 'mercadopago' as const,
      accountInfo: {
        userId: maskUserId(persisted.userId),
        siteId: persisted.siteId || 'MLA',
        externalStoreId: persisted.externalStoreId || generateExternalStoreId(cleanId),
        storeId: persisted.storeId || 'Configurada',
        externalPosId: persisted.externalPosId || generateExternalPosId(cleanId),
        posId: persisted.posId || 'Configurada',
        connectedAt: persisted.connectedAt,
        accountEmail: persisted.accountEmail,
        accountNickname: persisted.accountNickname || 'Cuenta vinculada correctamente',
      },
      lastError: undefined,
    };
  }
  ...
  public getSanitizedStatus(businessId: string = 'default') {
    ...
    if (!provider) {
      ...
      if (persisted && persisted.status === 'CONNECTED' && persisted.accessToken) {
        provider = {
          id: `mpp_${cleanId}`,
          businessId: cleanId,
          provider: 'mercadopago',
        ...
        this.providers.set(cleanId, provider);
      } else if (persisted && (persisted.status === 'DISCONNECTED' || !persisted.accessToken)) {
        return {
          enabled: false,
          connected: false,
          status: 'DISCONNECTED' as const,
    ...
    const isConnected = Boolean(provider && provider.status === 'CONNECTED');
    const isEnabled = isConnected ? (tenantConfig.enabled !== false) : false;
    ...
    return {
      ...
      provider: 'mercadopago' as const,
      accountInfo: {
        userId: maskUserId(provider?.userId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.userId : tenantConfig.productionConfig.userId)),
        siteId: provider?.siteId || 'MLA',
        externalStoreId: provider?.externalStoreId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.externalStoreId : tenantConfig.productionConfig.externalStoreId) || generateExternalStoreId(cleanId),
        storeId: provider?.storeId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.storeId : tenantConfig.productionConfig.storeId) || 'Configurada',
        externalPosId: provider?.externalPosId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.externalPosId : tenantConfig.productionConfig.externalPosId) || generateExternalPosId(cleanId),
        posId: provider?.posId || (tenantConfig.mode === 'TEST' ? tenantConfig.testConfig.posId : tenantConfig.productionConfig.posId) || 'Configurada',
        connectedAt: provider?.connectedAt || tenantConfig.updatedAt,
        accountEmail: provider?.accountEmail,
        accountNickname: provider?.accountNickname || 'Cuenta vinculada correctamente',
      },
      lastError: provider?.lastError,
    };
  }
  ...
  public async connectBusiness(params: {
    ...
    const existingPersisted = (await persistenceService.getConnectionAsync(cleanId)) || persistenceService.getConnection(cleanId);
    ...
    // Persist securely to Firestore primary and memory/disk backup
  ...
  /**
   * Disconnects Mercado Pago for a specific business asynchronously.
   * Strictly marks as DISCONNECTED in Firestore and clears active tokens.
   */
  public async disconnectBusinessAsync(businessId: string): Promise<{ success: boolean; message: string }> {
    const cleanId = String(businessId || '').trim();
    if (!cleanId) {
    ...
    // Clear tokens in token service and persistence asynchronously
    await mercadopagoTokenService.clearTokensAsync(cleanId);
    ...
    tenantConfigStore.updateConnectionStatus(cleanId, 'DISCONNECTED', 'Desconectado por el usuario');
