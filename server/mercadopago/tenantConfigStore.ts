...
class TenantConfigStoreManager {
  ...
  public getConfig(businessId: string = 'default'): TenantMercadoPagoConfig {
    ...
    if (!tenant) {
      ...
      if (persisted) {
        const isConnected = persisted.status === 'CONNECTED' && Boolean(persisted.accessToken);
        const isTenantEnabled = persisted.status === 'DISCONNECTED'
          ? false
          : (persisted.enabled !== undefined
            ? Boolean(persisted.enabled)
            : (persisted.explicitEnabled ? Boolean(persisted.enabled) : isGlobalEnabled));
        const tenantMode: MercadoPagoMode = (persisted.mode === 'TEST' || persisted.mode === 'PRODUCTION')
          ? persisted.mode
          : 'PRODUCTION';
        tenant = {
          businessId: key,
          enabled: isTenantEnabled,
          explicitEnabled: persisted.explicitEnabled ?? (persisted.enabled !== undefined),
          mode: tenantMode,
          autoConfirm: isGlobalAutoConfirm,
          connectionStatus: isConnected ? 'CONNECTED' : (isTenantEnabled ? 'NOT_VERIFIED' : 'DISCONNECTED'),
          lastVerification: persisted.updatedAt || persisted.connectedAt,
          lastVerificationMessage: isConnected ? 'Conexión activa guardada' : (isTenantEnabled ? 'Habilitado por configuración' : 'Desactivado'),
          testConfig: {
  ...
  /**
   * Asynchronously resolves tenant configuration by loading merchant connection from Firestore.
   * Crucial for serverless cold-start execution where memory is uninitialized.
   */
  public async getConfigAsync(businessId: string = 'default'): Promise<TenantMercadoPagoConfig> {
    const key = String(businessId || 'default').trim();
    const isDefault = key === 'default';
    const enabledStr = process.env.MERCADOPAGO_ENABLED?.toLowerCase().trim();
    const autoConfirmStr = process.env.MERCADOPAGO_AUTO_CONFIRM?.toLowerCase().trim();
    ...
    // Hydrate persisted connection from Firestore primary (or memory cache)
    const persisted = await persistenceService.getConnectionAsync(key);
    ...
    if (!tenant) {
      ...
      // Sync credentials if persisted exists, but respect explicit in-memory user overrides
      if (persisted) {
        if (persisted.mode === 'TEST' || persisted.mode === 'PRODUCTION') {
          tenant.mode = persisted.mode;
        }
        if (persisted.status === 'DISCONNECTED') {
          tenant.enabled = false;
          tenant.explicitEnabled = true;
          tenant.connectionStatus = 'DISCONNECTED';
          tenant.productionConfig.accessToken = '';
        } else {
          if (tenant.explicitEnabled === undefined && persisted.enabled !== undefined) {
            tenant.enabled = Boolean(persisted.enabled);
            tenant.explicitEnabled = true;
          }
          if (persisted.accessToken && !tenant.productionConfig.accessToken) {
            tenant.productionConfig.accessToken = persisted.accessToken;
          }
          if (persisted.userId && !tenant.productionConfig.userId) {
            tenant.productionConfig.userId = persisted.userId;
          }
        }
      }
      if (tenant.explicitEnabled === undefined) {
  ...
  /**
   * Returns a sanitized DTO safe for the frontend (asynchronous).
   * Hydrates from Firestore if needed.
   */
  public async getSanitizedConfigAsync(businessId: string = 'default'): Promise<SanitizedTenantMercadoPagoConfig> {
    const config = await this.getConfigAsync(businessId);
    const testHasToken = Boolean(
      (config.testConfig.accessToken && config.testConfig.accessToken.length > 5) ||
      (process.env.MERCADOPAGO_ACCESS_TOKEN && process.env.MERCADOPAGO_ACCESS_TOKEN.length > 5)
    );
  ...
  public saveConfig(
    ...
    // Persist to persistenceService so changes survive cold starts and reloads
    const existingConn = persistenceService.getConnection(businessId);
    const connectionToSave: StoredMerchantConnection = {
      ...(existingConn || {}),
      businessId,
      provider: 'mercadopago',
      status: existingConn?.status || (current.productionConfig.accessToken ? 'CONNECTED' : 'DISCONNECTED'),
      enabled: current.enabled,
      explicitEnabled: true,
      mode: current.mode,
      userId: current.productionConfig.userId || existingConn?.userId || (current.mode === 'TEST' ? TEST_SANDBOX_DEFAULTS.userId : ''),
      siteId: current.productionConfig.siteId || existingConn?.siteId || 'MLA',
      externalStoreId: current.productionConfig.externalStoreId || existingConn?.externalStoreId,
      externalPosId: current.productionConfig.externalPosId || existingConn?.externalPosId,
      storeId: current.productionConfig.storeId || existingConn?.storeId,
      posId: current.productionConfig.posId || existingConn?.posId,
      accessToken: current.productionConfig.accessToken || existingConn?.accessToken,
      accountNickname: existingConn?.accountNickname || (current.mode === 'TEST' ? 'Cuenta Sandbox / Prueba' : 'Cuenta Comercial'),
      accountEmail: existingConn?.accountEmail,
      connectedAt: existingConn?.connectedAt || new Date().toISOString(),
      updatedAt: current.updatedAt,
    };
    persistenceService.saveConnection(connectionToSave);
  ...
  /**
   * Asynchronously saves updated tenant configuration to Firestore + disk.
   */
  public async saveConfigAsync(
  ...
  ): Promise<TenantMercadoPagoConfig> {
    const current = await this.getConfigAsync(businessId);
    ...
    // Persist to Firestore + Disk
    const existingConn = await persistenceService.getConnectionAsync(businessId);
    const connectionToSave: StoredMerchantConnection = {
      ...(existingConn || {}),
      businessId,
      provider: 'mercadopago',
    ...
    await persistenceService.saveConnectionAsync(connectionToSave);
  ...
  /**
   * Asynchronously resolves active MercadoPagoConfig for runtime operations (orders, webhooks, validations).
   * Fully isolates TEST from PRODUCTION.
   * Leverages mercadopagoTokenService as the single authoritative source of valid/refreshed tokens.
   */
  public async getActiveRuntimeConfigAsync(businessId?: string): Promise<MercadoPagoConfig> {
    const cleanId = String(businessId || 'default').trim();
    const isDefaultTenant = (!businessId || cleanId === 'default');

    // 1. Ensure tenant config is hydrated from Firestore primary
    const tenant = await this.getConfigAsync(cleanId);
    const isTest = tenant.mode === 'TEST';
    const activeCreds = isTest ? tenant.testConfig : tenant.productionConfig;

    // 2. Token resolution:
    // In TEST mode: use testConfig or test env var
    // In PRODUCTION mode: Single Source of Truth via mercadopagoTokenService
    let token = '';
    if (isTest) {
      token = (activeCreds.accessToken || process.env.MERCADOPAGO_ACCESS_TOKEN || '').trim();
    } else {
      const { mercadopagoTokenService } = await import('./mercadopagoTokenService.js');
      const validToken = await mercadopagoTokenService.getValidAccessToken(cleanId);
      token = (validToken || activeCreds.accessToken || '').trim();
      if (token && activeCreds.accessToken !== token) {
        activeCreds.accessToken = token;
      }
    }
