...
const CACHE_VERSION = 'uwi-v1.0.1';
...
self.addEventListener('install', (event) => {
  ...
  // Activate immediately so clients take the updated bundle and purge obsolete caches
  self.skipWaiting();
});
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) => {
      return Promise.all(
        keys.map((key) => {
          // Delete previous uwi and legacy caches that do not match the current version
          const isUwiOrLegacyCache = key.startsWith('uwi-') || key.startsWith('mm-');
          if (isUwiOrLegacyCache && key !== STATIC_CACHE_NAME && key !== DYNAMIC_CACHE_NAME) {
