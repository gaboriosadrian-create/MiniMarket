...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchProfileAndBusiness = async (authUser: User): Promise<{ profile: UserProfile | null; errorMsg?: string }> => {
    try {
      ...
      let matchedProvisionalDocId: string | null = null;
      const now = new Date().toISOString();
      const authEmail = authUser.email ? authUser.email.toLowerCase().trim() : '';

      if (userSnap.exists()) {
        profileData = userSnap.data() as UserProfile;
      } else if (authEmail) {
        // Fallback: search by email to link existing pre-provisioned or invited user profile
        const q = query(collection(db, 'users'), where('email', '==', authEmail));
        const querySnap = await getDocs(q);
        if (!querySnap.empty) {
          const docSnap = querySnap.docs[0];
          const existingDocData = docSnap.data() as UserProfile;
          matchedProvisionalDocId = docSnap.id;
          ...
          // Clean up provisional doc if it was saved under a temporary key
          if (matchedProvisionalDocId && matchedProvisionalDocId !== authUser.uid) {
            try {
              await deleteDoc(doc(db, 'users', matchedProvisionalDocId));
            } catch (cleanupErr) {
              console.warn('Could not remove temporary invitation doc:', cleanupErr);
            }
          }
        }
      }

      // Check special SuperAdmin configuration for project owner
      const superAdminOwnerEmail = (import.meta.env.VITE_SUPERADMIN_EMAIL || 'gaboriosadrian@gmail.com').toLowerCase().trim();
      const isOwnerGoogleAccount = authEmail && authEmail === superAdminOwnerEmail;
      ...
      if (!profileData) {
        ...
        return {
          profile: null,
          errorMsg: 'Esta cuenta de Google no se encuentra autorizada para administrar ningún comercio en Uwi. Contactá al Super Administrador.'
        };
      }

      // Reject if user is blocked or disabled / inactive
      if (profileData.active === false || profileData.status === 'BLOCKED' || profileData.status === 'DISABLED' || profileData.invitationStatus === 'CANCELLED') {
        await signOut(auth);
        setUserProfile(null);
        setBusiness(null);
        ...
        return {
          ...
          errorMsg: isBlocked
            ...
            : 'Tu cuenta de usuario se encuentra desactivada. Contactá al administrador.'
        };
      }

      // Ensure business adminUserIds list contains the official authUser.uid and removes provisional IDs
      if (profileData.businessId) {
        try {
          const bizRef = doc(db, 'businesses', profileData.businessId);
          
          // 1. Add real Firebase Auth UID (idempotent with arrayUnion)
          await updateDoc(bizRef, {
            adminUserIds: arrayUnion(authUser.uid),
            updatedAt: now
          });

          // 2. Remove provisional identifier(s) if transitioning from a pre-authorization
          const provisionalKeysToRemove = [
            matchedProvisionalDocId,
            `auth_${profileData.businessId}_${authEmail.replace(/[^a-zA-Z0-9]/g, '_')}`,
            `inv_${profileData.businessId}_${authEmail.replace(/[^a-zA-Z0-9]/g, '_')}`
          ].filter((k): k is string => Boolean(k && k !== authUser.uid));

          if (provisionalKeysToRemove.length > 0) {
            await updateDoc(bizRef, {
              adminUserIds: arrayRemove(...provisionalKeysToRemove)
            });
          }
        } catch (bErr) {
          console.warn('Could not update business adminUserIds array:', bErr);
        }
      }
