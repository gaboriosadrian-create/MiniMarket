...
export async function settlePaymentObligation(input: SettleObligationInput): Promise<PaymentSettlement> {
  ...
  // Enforce mandatory OPEN cash session if paying with CASH from register
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // If paid with CASH (Dinero de Caja del negocio), create CashMovement with deterministic docId & session info
    if (input.fundSource === 'CASH' && activeSession) {
      const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'SETTLEMENT', settlementId);
