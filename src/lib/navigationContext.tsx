...
export const NavigationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerBackAction = useCallback((handler: BackActionHandler) => {
    let wasExecutedByPopstate = false;
    const wrappedHandler: BackActionHandler = () => {
      wasExecutedByPopstate = true;
      return handler();
    };

    backHandlersRef.current.push(wrappedHandler);
    ...
    return () => {
      const idx = backHandlersRef.current.indexOf(wrappedHandler);
      if (idx !== -1) {
        backHandlersRef.current.splice(idx, 1);
      }
      // If closed via UI (and NOT via popstate event), pop the dummy history entry so history does not accumulate
      if (!wasExecutedByPopstate) {
        try {
  ...
  useEffect(() => {
    const handlePopState = (e: PopStateEvent) => {
      // 0. If a pop was triggered programmatically due to UI unregister, consume it
