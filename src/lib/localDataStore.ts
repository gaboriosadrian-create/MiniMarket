import { Product, Sale, Expense, StockAdjustment, Receiving, Purchase, OutboxOperation, OutboxStatus } from '../types';
 ...
 * 8. Offline Purchases & Snapshots (offline_purchases)
 */
...
class LocalDataStore {
  ...
  /**
   * Logs current IndexedDB state and available stores
   */
  public logDatabaseState(db: IDBDatabase): void {
    console.log(`[IndexedDB] Database: ${db.name}`);
    console.log(`[IndexedDB] Version: ${db.version}`);
    console.log(`[IndexedDB] Stores:`);
  ...
  /**
   * Checks and returns stores from candidate list that actually exist in the database,
   * logging the existence of requested stores for diagnostic tracking.
   */
  public getAvailableStores(db: IDBDatabase, candidateStores: string[], operationName?: string): string[] {
    const available: string[] = [];
    for (const storeName of candidateStores) {
      const exists = db.objectStoreNames.contains(storeName);
      if (operationName) {
        console.log(`[IndexedDB] ${operationName} | Requested store: ${storeName} | Store exists: ${exists}`);
      }
      if (exists) {
        available.push(storeName);
      }
    }
    return available;
  }
  ...
  /**
   * Configures all required object stores and indexes defensively during onupgradeneeded
   */
  private configureStoresAndIndexes(db: IDBDatabase, transaction: IDBTransaction | null): void {
    // 1. Products Store
    if (!db.objectStoreNames.contains('products')) {
      ...
      if (!store.indexNames.contains('by_businessId')) {
        store.createIndex('by_businessId', 'businessId', { unique: false });
      }
      if (!store.indexNames.contains('by_business_barcode')) {
        store.createIndex('by_business_barcode', ['businessId', 'barcode'], { unique: false });
      }
      if (!store.indexNames.contains('by_business_active')) {
        store.createIndex('by_business_active', ['businessId', 'active'], { unique: false });
      }
    }

    // 2. Business Metadata Store
    if (!db.objectStoreNames.contains('business_meta')) {
      db.createObjectStore('business_meta', { keyPath: 'businessId' });
    }

    // 3. Outbox Operations Store
    if (!db.objectStoreNames.contains('outbox')) {
      ...
      if (!store.indexNames.contains('by_business_status')) {
        store.createIndex('by_business_status', ['businessId', 'status'], { unique: false });
      }
      if (!store.indexNames.contains('by_business_createdAt')) {
        store.createIndex('by_business_createdAt', ['businessId', 'createdAt'], { unique: false });
      }
      if (!store.indexNames.contains('by_saleId')) {
        store.createIndex('by_saleId', 'saleId', { unique: false });
      }
    }

    // 4. Offline Sales History Store
    if (!db.objectStoreNames.contains('offline_sales')) {
      ...
      if (!store.indexNames.contains('by_business_syncStatus')) {
        store.createIndex('by_business_syncStatus', ['businessId', 'syncStatus'], { unique: false });
      }
    }

    // 5. Offline Expenses Store
    ...
    // 6. Offline Stock Adjustments Store
    if (!db.objectStoreNames.contains('offline_stock_adjustments')) {
      ...
      const store = transaction.objectStore('offline_stock_adjustments');
      if (!store.indexNames.contains('by_businessId')) {
        store.createIndex('by_businessId', 'businessId', { unique: false });
      }
      if (!store.indexNames.contains('by_business_createdAt')) {
    ...
    if (!db.objectStoreNames.contains('offline_receivings')) {
      ...
      const store = transaction.objectStore('offline_receivings');
      if (!store.indexNames.contains('by_businessId')) {
        store.createIndex('by_businessId', 'businessId', { unique: false });
      }
      if (!store.indexNames.contains('by_business_createdAt')) {
    ...
    // 8. Offline Purchases Store
    if (!db.objectStoreNames.contains('offline_purchases')) {
      const purchaseStore = db.createObjectStore('offline_purchases', { keyPath: 'id' });
      purchaseStore.createIndex('by_businessId', 'businessId', { unique: false });
      purchaseStore.createIndex('by_business_createdAt', ['businessId', 'createdAt'], { unique: false });
      purchaseStore.createIndex('by_business_syncStatus', ['businessId', 'syncStatus'], { unique: false });
    } else if (transaction) {
      const store = transaction.objectStore('offline_purchases');
      if (!store.indexNames.contains('by_businessId')) {
        store.createIndex('by_businessId', 'businessId', { unique: false });
      }
      if (!store.indexNames.contains('by_business_createdAt')) {
  ...
  /**
   * Helper to perform dynamic version upgrade when missing object stores are detected
   */
  private async upgradeDatabaseToVersion(targetVersion: number): Promise<IDBDatabase> {
    return new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        console.log(`[IndexedDB] Actualizando esquema a versión ${targetVersion}...`);
        this.configureStoresAndIndexes(db, tx);
      };

      request.onsuccess = () => {
        const db = request.result;
        this.logDatabaseState(db);
        resolve(db);
      };

      request.onerror = () => {
        reject(request.error || new Error(`Error al actualizar IndexedDB a versión ${targetVersion}`));
      };
    });
  }

  /**
   * Open / initialize the IndexedDB instance with auto-healing and schema validation
   */
  private async getDB(): Promise<IDBDatabase> {
    ...
    this.dbPromise = new Promise<IDBDatabase>((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        const db = (event.target as IDBOpenDBRequest).result;
        const tx = (event.target as IDBOpenDBRequest).transaction;
        console.log(`[IndexedDB] onupgradeneeded: DB ${db.name} pasando a v${DB_VERSION}`);
        this.configureStoresAndIndexes(db, tx);
      };
      ...
      request.onsuccess = async () => {
        ...
        // Self-Healing: verify if all required stores exist in opened DB
        const missingStores = REQUIRED_OBJECT_STORES.filter((s) => !db.objectStoreNames.contains(s));
        if (missingStores.length > 0) {
          console.warn(`[IndexedDB] Falta(n) store(s) requerido(s): ${missingStores.join(', ')}. Iniciando auto-migración no destructiva a v${db.version + 1}...`);
          db.close();
          this.dbPromise = null;
          try {
            const upgradedDb = await this.upgradeDatabaseToVersion(db.version + 1);
            resolve(upgradedDb);
          } catch (upgradeErr) {
            reject(upgradeErr);
          }
  ...
  public async saveProducts(
    ...
    return new Promise<void>((resolve, reject) => {
      const availableStores = this.getAvailableStores(db, ['products', 'business_meta'], 'saveProducts');
      if (!availableStores.includes('products')) {
        return resolve();
      }
      const tx = db.transaction(availableStores, 'readwrite');
      const productStore = tx.objectStore('products');
      const metaStore = availableStores.includes('business_meta') ? tx.objectStore('business_meta') : null;
  ...
  public async createOfflineSaleTransaction(
    ...
    return new Promise<Sale>((resolve, reject) => {
      const candidateStores = ['products', 'offline_sales', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'createOfflineSaleTransaction');
      
      // All 3 stores are strictly MANDATORY for an atomic offline sale
      const missingMandatory = candidateStores.filter(s => !availableStores.includes(s));
      if (missingMandatory.length > 0) {
        return reject(new Error(`Almacén(es) obligatorio(s) no disponible(s) en IndexedDB: ${missingMandatory.join(', ')}`));
      }
  ...
  public async recoverStuckSyncingOperations(
    ...
    try {
      ...
      return new Promise<number>((resolve, reject) => {
        const candidateStores = ['outbox', 'offline_sales', 'offline_expenses', 'offline_stock_adjustments', 'offline_receivings'];
        const availableStores = this.getAvailableStores(db, candidateStores, 'recoverStuckSyncingOperations');
        if (!availableStores.includes('outbox')) {
          return resolve(0);
        }

        const tx = db.transaction(availableStores, 'readwrite');
        const outboxStore = tx.objectStore('outbox');
        const salesStore = availableStores.includes('offline_sales') ? tx.objectStore('offline_sales') : null;
        const expenseStore = availableStores.includes('offline_expenses') ? tx.objectStore('offline_expenses') : null;
        const adjStore = availableStores.includes('offline_stock_adjustments') ? tx.objectStore('offline_stock_adjustments') : null;
        const rcvStore = availableStores.includes('offline_receivings') ? tx.objectStore('offline_receivings') : null;
  ...
  public async resolveOfflineOperation(params: {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['outbox', 'offline_sales'], 'resolveOfflineOperation');
        if (!availableStores.includes('outbox')) {
          return resolve({ matchedBy: 'NONE' });
        }
          ...
          if (targetSaleId && outboxStore.indexNames.contains('by_saleId')) {
            ...
            req.onsuccess = () => {
              ...
              if (found) {
                ...
                if (!matchedSale && found.saleId && salesStore) {
  ...
  public async cancelOfflineSaleAndRevertStock(
    ...
    return new Promise((resolve, reject) => {
      const candidateStores = ['products', 'offline_sales', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'cancelOfflineSaleAndRevertStock');
      const missingMandatory = candidateStores.filter(s => !availableStores.includes(s));
      if (missingMandatory.length > 0) {
        return reject(new Error(`Almacén(es) obligatorio(s) no disponible(s) en IndexedDB: ${missingMandatory.join(', ')}`));
      }
  ...
  public async updateOutboxOperationStatus(
    ...
    try {
      ...
      return new Promise<void>((resolve, reject) => {
        const candidateStores = ['outbox', 'offline_sales', 'offline_expenses', 'offline_stock_adjustments', 'offline_receivings'];
        const availableStores = this.getAvailableStores(db, candidateStores, 'updateOutboxOperationStatus');
        if (!availableStores.includes('outbox')) {
          return resolve();
        }
          ...
          if (op.expenseId && expenseStore) {
            ...
            expReq.onsuccess = () => {
              ...
              if (e && e.businessId === businessId) {
                e.syncStatus = status;
                if (syncedAt) e.syncStatus = status;
        ...
        function fallbackScan() {
          ...
          scanReq.onsuccess = () => {
            ...
            if (found) {
              applyUpdates(found);
            } else if ((saleId || operationId) && salesStore) {
  ...
  public async cancelOfflineStockAdjustmentAndRevertStock(
    ...
    const db = await this.getDB();
    return new Promise<void>((resolve, reject) => {
      const candidateStores = ['products', 'offline_stock_adjustments', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'cancelOfflineStockAdjustmentAndRevertStock');
      const tx = db.transaction(availableStores, 'readwrite');
      const productStore = availableStores.includes('products') ? tx.objectStore('products') : null;
      const adjStore = availableStores.includes('offline_stock_adjustments') ? tx.objectStore('offline_stock_adjustments') : null;
      const outboxStore = availableStores.includes('outbox') ? tx.objectStore('outbox') : null;

      tx.onerror = () => reject(tx.error);
      tx.oncomplete = () => resolve();

      if (!adjStore) return resolve();
  ...
  public async createOfflineReceivingTransaction(
    ...
    return new Promise<Receiving>((resolve, reject) => {
      const candidateStores = ['products', 'offline_receivings', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'createOfflineReceivingTransaction');
      const missingMandatory = candidateStores.filter(s => !availableStores.includes(s));
      if (missingMandatory.length > 0) {
        return reject(new Error(`Almacén(es) obligatorio(s) no disponible(s) en IndexedDB: ${missingMandatory.join(', ')}`));
      }
      ...
      pReq.onsuccess = () => {
        ...
        for (const item of receiving.items) {
          ...
          if (!prod) {
            tx.abort();
            return reject(new Error(`El producto recibido "${item.productName}" no existe en el catálogo local.`));
          }
  ...
  public async saveOfflineReceiving(receiving: Receiving): Promise<void> {
    ...
    try {
      const db = await this.getDB();
      return new Promise<void>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['offline_receivings'], 'saveOfflineReceiving');
        if (!availableStores.includes('offline_receivings')) return resolve();
  ...
  public async getOfflineReceivingsByBusiness(businessId: string): Promise<Receiving[]> {
    ...
    try {
      const db = await this.getDB();
      return new Promise<Receiving[]>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['offline_receivings'], 'getOfflineReceivingsByBusiness');
        if (!availableStores.includes('offline_receivings')) return resolve([]);
  ...
  public async cancelOfflineReceivingAndRevertStock(
    ...
    return new Promise<void>((resolve, reject) => {
      const candidateStores = ['products', 'offline_receivings', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'cancelOfflineReceivingAndRevertStock');
      const tx = db.transaction(availableStores, 'readwrite');
      const productStore = availableStores.includes('products') ? tx.objectStore('products') : null;
      const rcvStore = availableStores.includes('offline_receivings') ? tx.objectStore('offline_receivings') : null;
      const outboxStore = availableStores.includes('outbox') ? tx.objectStore('outbox') : null;
  ...
  // ==========================================
  // 4.5 OFFLINE PURCHASES
  // ==========================================

  /**
   * Atomically saves an offline purchase and creates an outbox operation.
   * If isImmediateDelivery is true, increments local stock in IndexedDB for each purchased item.
   */
  public async createOfflinePurchaseTransaction(
    businessId: string,
    purchase: Purchase,
    operation: OutboxOperation
  ): Promise<Purchase> {
    if (!this.isSupported()) {
      throw new Error('IndexedDB no está soportado en este dispositivo.');
    }

    const db = await this.getDB();
    return new Promise<Purchase>((resolve, reject) => {
      const candidateStores = ['products', 'offline_purchases', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'createOfflinePurchaseTransaction');
      
      if (!availableStores.includes('offline_purchases') || !availableStores.includes('outbox')) {
        return reject(new Error('Los object stores necesarios (offline_purchases, outbox) no están disponibles.'));
      }

      const tx = db.transaction(availableStores, 'readwrite');
      const productStore = availableStores.includes('products') ? tx.objectStore('products') : null;
      const purchaseStore = tx.objectStore('offline_purchases');
      const outboxStore = tx.objectStore('outbox');

      tx.onerror = () => reject(tx.error || new Error('Error en transacción de compra offline'));
      ...
      if (productStore) {
        ...
        pReq.onsuccess = () => {
          ...
          // Increment local stock and/or update costPrice for each purchased product
          for (const item of purchase.items) {
            const prod = pMap.get(item.productId);
            if (prod) {
              let updated = false;
              if (purchase.isImmediateDelivery && prod.tracksStock !== false) {
                const currentStock = Number(prod.stock) || 0;
                const qty = Math.floor(Number(item.quantity)) || 0;
  ...
  /**
   * Save an offline purchase snapshot locally
   */
  public async saveOfflinePurchase(purchase: Purchase): Promise<void> {
    if (!this.isSupported() || !purchase.id || !purchase.businessId) return;

    try {
      const db = await this.getDB();
      return new Promise<void>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['offline_purchases'], 'saveOfflinePurchase');
        if (!availableStores.includes('offline_purchases')) return resolve();
        const tx = db.transaction('offline_purchases', 'readwrite');
        const store = tx.objectStore('offline_purchases');
        store.put(purchase);
        tx.oncomplete = () => resolve();
        tx.onerror = () => reject(tx.error);
      });
    } catch (err) {
      console.warn('[LocalDataStore] Error guardando compra local:', err);
    }
  }

  /**
   * Get all offline purchases for a business
   */
  public async getOfflinePurchasesByBusiness(businessId: string): Promise<Purchase[]> {
    if (!this.isSupported() || !businessId) return [];

    try {
      const db = await this.getDB();
      return new Promise<Purchase[]>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['offline_purchases'], 'getOfflinePurchasesByBusiness');
        if (!availableStores.includes('offline_purchases')) return resolve([]);
        const tx = db.transaction('offline_purchases', 'readonly');
        const store = tx.objectStore('offline_purchases');
        const index = store.index('by_businessId');
        const request = index.getAll(IDBKeyRange.only(businessId));

        request.onsuccess = () => {
          const list = (request.result || []) as Purchase[];
          list.sort((a, b) => {
            const tA = a.createdAt ? new Date(a.createdAt).getTime() : 0;
            const tB = b.createdAt ? new Date(b.createdAt).getTime() : 0;
            return tB - tA;
      ...
      console.warn('[LocalDataStore] Error obteniendo compras locales:', err);
      return [];
    }
  }

  /**
   * Get a single offline purchase by ID
   */
  public async getOfflinePurchaseById(purchaseId: string): Promise<Purchase | null> {
    if (!this.isSupported() || !purchaseId) return null;

    try {
      const db = await this.getDB();
      return new Promise<Purchase | null>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['offline_purchases'], 'getOfflinePurchaseById');
        if (!availableStores.includes('offline_purchases')) return resolve(null);
        const tx = db.transaction('offline_purchases', 'readonly');
        const store = tx.objectStore('offline_purchases');
        const request = store.get(purchaseId);

        request.onsuccess = () => resolve((request.result as Purchase) || null);
        request.onerror = () => reject(request.error);
      });
    } catch (err) {
      console.warn('[LocalDataStore] Error buscando compra local por ID:', err);
      return null;
    }
  }

  /**
   * Cancels an offline purchase and reverts incremented stock if immediate delivery
   */
  public async cancelOfflinePurchaseAndRevertStock(
    businessId: string,
    purchaseId: string,
    operationId: string,
    reason: string = 'Anulada por usuario'
  ): Promise<void> {
    if (!this.isSupported() || !businessId) return;
    ...
    return new Promise<void>((resolve, reject) => {
      const candidateStores = ['products', 'offline_purchases', 'outbox'];
      const availableStores = this.getAvailableStores(db, candidateStores, 'cancelOfflinePurchaseAndRevertStock');
      const tx = db.transaction(availableStores, 'readwrite');
      const productStore = availableStores.includes('products') ? tx.objectStore('products') : null;
      const purchaseStore = availableStores.includes('offline_purchases') ? tx.objectStore('offline_purchases') : null;
      const outboxStore = availableStores.includes('outbox') ? tx.objectStore('outbox') : null;

      tx.onerror = () => reject(tx.error);
      tx.oncomplete = () => resolve();

      if (!purchaseStore) return resolve();

      const pReq = purchaseStore.get(purchaseId);
      pReq.onsuccess = () => {
        const purchase = pReq.result as Purchase | undefined;
        if (!purchase || purchase.businessId !== businessId) return;

        if (purchase.isImmediateDelivery && productStore) {
          const prodIndex = productStore.index('by_businessId');
          const prodReq = prodIndex.getAll(IDBKeyRange.only(businessId));

          prodReq.onsuccess = () => {
            const prods = (prodReq.result || []) as Product[];
            const pMap = new Map<string, Product>();
            prods.forEach(p => pMap.set(p.id, { ...p }));
            ...
            purchase.syncStatus = 'CANCELLED';
            purchase.syncError = `Compra cancelada: ${reason}`;
            purchaseStore.put(purchase);
            ...
            if (outboxStore) {
              ...
              opReq.onsuccess = () => {
                ...
                if (op && op.businessId === businessId) {
                  op.status = 'CANCELLED';
                  op.lastError = `Compra cancelada localmente: ${reason}`;
                  if (op.purchaseSnapshot) {
                    op.purchaseSnapshot.status = 'CANCELLED';
                    op.purchaseSnapshot.syncStatus = 'CANCELLED';
                  }
                  outboxStore.put(op);
                }
              };
  ...
  public async saveBusinessMetadata(meta: BusinessMetadata): Promise<void> {
    ...
    try {
      const db = await this.getDB();
      return new Promise<void>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['business_meta'], 'saveBusinessMetadata');
        if (!availableStores.includes('business_meta')) return resolve();
  ...
  public async getBusinessMetadata(businessId: string): Promise<BusinessMetadata | null> {
    ...
    try {
      ...
      return new Promise<BusinessMetadata | null>((resolve, reject) => {
        const availableStores = this.getAvailableStores(db, ['business_meta'], 'getBusinessMetadata');
        if (!availableStores.includes('business_meta')) return resolve(null);
  ...
  public async clearBusinessCatalog(businessId: string): Promise<void> {
    ...
    try {
      const db = await this.getDB();
      return new Promise<void>((resolve, reject) => {
        const candidateStores = ['products', 'business_meta'];
        const availableStores = this.getAvailableStores(db, candidateStores, 'clearBusinessCatalog');
        if (availableStores.length === 0) return resolve();
        const tx = db.transaction(availableStores, 'readwrite');
        const productStore = availableStores.includes('products') ? tx.objectStore('products') : null;
        const metaStore = availableStores.includes('business_meta') ? tx.objectStore('business_meta') : null;
    ...
    try {
      ...
      return new Promise<void>((resolve, reject) => {
        const candidateStores = [
          'products', 'business_meta', 'outbox', 'offline_sales', 'offline_expenses', 'offline_stock_adjustments', 'offline_receivings', 'offline_purchases'
        ];
        const availableStores = this.getAvailableStores(db, candidateStores, 'clearBusinessData');
        if (availableStores.length === 0) return resolve();
        ...
        // 8. Offline Purchases
        if (purchaseStore) {
          const purIndex = purchaseStore.index('by_businessId');
          const purReq = purIndex.getAllKeys(IDBKeyRange.only(businessId));
          purReq.onsuccess = () => {
            (purReq.result || []).forEach((k) => purchaseStore.delete(k));
          };
        }
      });
    } catch (err) {
