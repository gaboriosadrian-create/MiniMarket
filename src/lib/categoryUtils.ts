...
/**
 * Normalizes text for header or category comparison:
 * lowercase, removes accents/diacritics, strips punctuation and collapses spaces.
 */
export function normalizeHeaderString(str: string | null | undefined): string {
  if (!str) return '';
  return String(str)
    .trim()
    .toLowerCase()
    ...
    .replace(/[._\-/\\]/g, ' ')       // turn dots, underscores, dashes, slashes to space
...
/**
 * Normalizes category and product names:
 * 1. Convert to string
 * 2. trim()
 * 3. Collapse multiple internal spaces to a single space
 * 4. Convert all to lowercase
 * 5. Convert strictly ONLY the first alphabetical character to uppercase
 *
 * Example: "BEBIDAS" -> "Bebidas"
 * Example: "  BEBIDAS  " -> "Bebidas"
 * Example: "  BEBIDAS   SIN ALCOHOL " -> "Bebidas sin alcohol"
 * Example: "AGUA MINERAL MANAOS 600ML" -> "Agua mineral manaos 600ml"
 * Example: "GOLOSINAS" -> "Golosinas"
 * Example: "ALMACEN" -> "Almacen"
 * Example: "LIMPIEZA" -> "Limpieza"
 */
export function normalizeCategoryName(text: string | null | undefined): string {
  if (text === null || text === undefined) return '';
  const trimmed = String(text).trim().replace(/\s+/g, ' ');
  if (!trimmed) return '';
  const lower = trimmed.toLowerCase();
  
  // Find index of the first alphabetical letter (including accented vowels & ñ)
  const match = lower.match(/[a-zà-ÿñ]/i);
...
export const normalizeDisplayText = normalizeCategoryName;
export const normalizeProductName = normalizeCategoryName;
...
// Find existing category ignoring casing, accents, and redundant spaces
export function findMatchingCategory(inputCategory: string, existingCategories: string[]): string | null {
  ...
  const normalizedInput = normalizeHeaderString(inputCategory);
  if (!normalizedInput) return null;

  for (const cat of existingCategories) {
    if (!cat) continue;
    if (normalizeHeaderString(cat) === normalizedInput) {
      return cat.trim();
    }
  }
  return null;
...
// Find existing supplier ignoring casing, accents, and redundant spaces
export function findMatchingSupplier(inputSupplier: string, existingSuppliers: string[]): string | null {
  if (!inputSupplier) return null;
  const normalizedInput = normalizeHeaderString(inputSupplier);
  if (!normalizedInput) return null;

  for (const sup of existingSuppliers) {
    if (!sup) continue;
    if (normalizeHeaderString(sup) === normalizedInput) {
      return sup.trim();
    }
  }
  return null;
...
export function getUniqueCategories(items: (string | { category?: string } | undefined | null)[]): string[] {
  const categoryMap = new Map<string, string>(); // normalizedKey -> canonicalName
  ...
  items.forEach((item) => {
    ...
    if (rawCategory) {
      ...
      if (clean) {
        const normKey = normalizeHeaderString(clean);
        if (!categoryMap.has(normKey)) {
          categoryMap.set(normKey, clean);
        }
      }
    }
  });
...
export function getUniqueSuppliers(items: (string | { supplierName?: string; supplier?: string } | undefined | null)[]): string[] {
  const supplierMap = new Map<string, string>(); // normalizedKey -> canonicalName

  items.forEach((item) => {
    const rawSupplier = typeof item === 'string' ? item : (item?.supplierName || item?.supplier);
    if (rawSupplier) {
      const clean = normalizeCategoryName(rawSupplier);
      if (clean) {
        const normKey = normalizeHeaderString(clean);
        if (!supplierMap.has(normKey)) {
          supplierMap.set(normKey, clean);
        }
      }
    }
  });
