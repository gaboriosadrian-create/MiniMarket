    ...
    if (existingOrder.businessRefId) {
      try {
        await logAdminAction({
          ...
          action: 'PROVEEDOR_CONFIRMO_SOLICITUD',
        ...
        await createNotification({
          ...
          title: `Proveedor confirmó pedido #${existingOrder.requestCode}`,
          message: `${existingOrder.supplierName || 'El proveedor'} confirmó ${totalUnitsConfirmed} de ${totalUnitsRequested} unidades (${completeCount} completos, ${partialCount} parciales, ${noStockCount} sin stock).`,
          linkTab: 'receiving',
          eventId: `prov_conf_adm_${existingOrder.requestCode}_${now.slice(0, 16)}`,
          metadata: {
            replenishmentId: existingOrder.orderRefId,
            requestCode: existingOrder.requestCode,
        ...
        // If requested by a specific seller, also notify that seller
        if (existingOrder.requestedBy && existingOrder.requestedBy !== 'ADMIN') {
          await createNotification({
            businessId: existingOrder.businessRefId,
            userId: existingOrder.requestedBy,
            type: 'PROVEEDOR_CONFIRMO_SOLICITUD',
            title: `Proveedor confirmó pedido #${existingOrder.requestCode}`,
            message: `${existingOrder.supplierName || 'El proveedor'} confirmó ${totalUnitsConfirmed} de ${totalUnitsRequested} unidades de tu solicitud. Ya puedes verificarla en Recepción.`,
            linkTab: 'receiving',
            eventId: `prov_conf_usr_${existingOrder.requestCode}_${now.slice(0, 16)}`,
            metadata: {
              replenishmentId: existingOrder.orderRefId,
              requestCode: existingOrder.requestCode,
              supplierName: existingOrder.supplierName,
...
export async function sharePublicOrderLink(
  ...
  const cleanBusiness = order.businessName || 'uwi';
