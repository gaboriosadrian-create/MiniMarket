...
import { normalizeText, getDefaultIconForCategoryOrProduct } from './categoryUtils';
import { 
  Product, 
  ComboItem,
  InventoryMovement, 
...
export async function createProduct(
  ...
  const icon = input.icon || getDefaultIconForCategoryOrProduct(input.name || category);
