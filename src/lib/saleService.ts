...
import { recordCashMovementFromOperation, getCurrentSession, getDeterministicCashMovementId } from './cashCoreService';
  ...
  if (requiresOnlineVerification) {
    ...
    if (input.paymentDetails?.externalReference && typeof window !== 'undefined' && window.fetch) {
      try {
        ...
        const resp = await fetch(getApiUrl('/api/mercadopago/validate-sale'), {
          method: 'POST',
...
async function executeOnlineFirestoreSale(
  ...
  // Validate cash session if sale involves physical cash
  const isCashSale = input.paymentMethod === 'EFECTIVO' || (input.paymentMethod === 'COMBINADO' && (input.paymentBreakdown?.cashAmount || 0) > 0);
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // 4. Record Cash Movement atomically if sale involves physical cash
    if (isCashSale && activeSession) {
      const cashAmount = input.paymentMethod === 'EFECTIVO'
        ? computedTotal
        : (input.paymentBreakdown?.cashAmount || 0);

      if (cashAmount > 0) {
        const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'SALE', saleId);
        ...
        transaction.set(cashMovRef, {
          ...
          flow: 'INCOME',
          amount: cashAmount,
          description: `Venta #${saleId.slice(-6).toUpperCase()}`,
          sourceType: 'SALE',
          sourceId: saleId,
          referenceType: 'SALE',
          referenceId: saleId,
...
export async function syncSaleOperationToFirestore(operation: OutboxOperation): Promise<{
  ...
  try {
    // Validate cash session if sale involves physical cash
    const isEfectivo = (operation.saleSnapshot?.paymentMethod || operation.payload?.paymentMethod) === 'EFECTIVO';
    const isCombinado = (operation.saleSnapshot?.paymentMethod || operation.payload?.paymentMethod) === 'COMBINADO';
    const pb = operation.saleSnapshot?.paymentBreakdown || operation.payload?.paymentBreakdown;
    const totalAmt = sanitizeNumber(operation.saleSnapshot?.total ?? operation.payload?.total, 0, 999999999, 0);
    const cashAmt = isEfectivo ? totalAmt : (isCombinado ? (sanitizeNumber(pb?.cashAmount, 0, 999999999, 0)) : 0);
    ...
    const syncResult = await runTransaction(db, async (transaction) => {
      ...
      // 5. Atomic Cash Movement if sale involves physical cash
      if (cashAmt > 0 && activeSession) {
        const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'SALE', saleId);
        const cashMovRef = doc(db, 'cash_movements', movementDocId);
        transaction.set(cashMovRef, {
          id: movementDocId,
          ...
          flow: 'INCOME',
          amount: cashAmt,
          description: `Venta Offline Sincronizada #${(saleId || '').slice(-6).toUpperCase()}`,
          sourceType: 'SALE',
          sourceId: saleId,
          referenceType: 'SALE',
          referenceId: saleId,
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (cashAmt > 0 && activeSession) {
        ...
        transaction.set(cashMovRef, {
          ...
          flow: 'INCOME',
          amount: cashAmt,
          description: `Venta Forzada Offline #${(saleId || '').slice(-6).toUpperCase()}`,
          sourceType: 'SALE',
          sourceId: saleId,
          referenceType: 'SALE',
          referenceId: saleId,
