...
import { Sale, SaleItem, Product, InventoryMovement, OutboxOperation, PaymentMethod, PaymentBreakdown } from '../types';
  ...
  const saleItemsSnapshot: SaleItem[] = input.items.map((item) => {
    ...
    if (item.product.isCombo) {
      ...
      if (Array.isArray(item.product.comboItems) && item.product.comboItems.length > 0) {
        itemSnapshot.comboItems = item.product.comboItems.map((c) => ({
          productId: sanitizeString(c.productId, 64),
          productName: sanitizeString(c.productName || '', 150),
          quantity: sanitizeInteger(c.quantity, 1, 999999, 1)
        }));
