...
export async function confirmPurchaseTransaction({
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (isPaidWithCash) {
      ...
      activeSession = await getCurrentSession(user.businessId);
    ...
    if (isPaidWithCash && activeSession) {
      const movementDocId = getDeterministicCashMovementId(user.businessId, 'PURCHASE', purchaseId);
...
export async function cancelPurchaseTransaction(input: CancelPurchaseInput): Promise<void> {
  ...
  // Check active session beforehand if purchase was paid with cash
  const purchaseRefPre = doc(db, 'purchases', cleanPurchaseId);
  const purchaseSnapPre = await getDoc(purchaseRefPre);
  if (!purchaseSnapPre.exists()) {
    throw new Error('La compra no existe.');
  }
  const purchasePre = purchaseSnapPre.data() as Purchase;
  const isPaidWithCashPre = purchasePre.paymentStatus === 'PAGADO' && purchasePre.fundSource === 'CASH' && purchasePre.paymentMethod === 'EFECTIVO';
  const purchaseTotalPre = Number(purchasePre.total || purchasePre.amount || 0);

  let activeSession: any = null;
  if (isPaidWithCashPre && purchaseTotalPre > 0) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (isPaidWithCash && purchaseTotal > 0 && activeSession) {
      const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'PURCHASE_CANCELLATION', `cancel_${cleanPurchaseId}`);
      const cashMovRef = doc(db, 'cash_movements', movementDocId);
      transaction.set(cashMovRef, {
        id: cashMovRef.id,
        businessId: cleanBusinessId,
        ...
        sourceId: `cancel_${cleanPurchaseId}`,
        referenceType: 'PURCHASE',
  ...
  return withActionLock(`sync_purchase_${purchaseId}`, async () => {
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (isPaidWithCash && purchaseTotal > 0 && activeSession) {
          const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'PURCHASE', purchaseId);
          const cashMovRef = doc(db, 'cash_movements', movementDocId);
          cashMovementId = movementDocId;
          transaction.set(cashMovRef, {
            id: cashMovementId,
