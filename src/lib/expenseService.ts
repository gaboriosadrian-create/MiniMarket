...
import { getCurrentSession, getDeterministicCashMovementId } from './cashCoreService';
...
export async function createExpense(expenseData: Omit<Expense, 'id'>): Promise<Expense> {
  ...
  try {
    ...
    if (isPaidWithCash) {
      activeSession = await getCurrentSession(cleanBusinessId);
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (isPaidWithCash && activeSession) {
        const movementDocId = getDeterministicCashMovementId(cleanBusinessId, 'EXPENSE', onlineId);
        const cashMovRef = doc(db, 'cash_movements', movementDocId);
        cashMovementId = movementDocId;
...
export async function payExpense(input: PayExpenseInput): Promise<Expense> {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (input.fundSource === 'CASH') {
      activeSession = await getCurrentSession(exp.businessId);
      if (!activeSession || activeSession.status !== 'OPEN') {
        throw new Error('NO HAY UNA SESIÓN DE CAJA ABIERTA PARA REGISTRAR EL MOVIMIENTO.');
      }
    }
    ...
    // Stable Payment Event ID (e.g. from input.paymentId, operationId, or deterministic index)
    const paymentEventId = input.paymentId || (input as any).operationId || `${cleanExpenseId}_pay_${newPaid}`;

    // 1. If paid with CASH, create CashMovement
    if (input.fundSource === 'CASH' && activeSession) {
      const movementDocId = getDeterministicCashMovementId(exp.businessId, 'EXPENSE', paymentEventId);
      const cashMovRef = doc(db, 'cash_movements', movementDocId);
      cashMovementId = movementDocId;
      transaction.set(cashMovRef, {
        id: cashMovementId,
...
export async function cancelExpense(input: CancelExpenseInput): Promise<Expense> {
  ...
  // Check active session beforehand if it was paid with cash
  const expSnapPre = await getDoc(expenseRef);
  if (!expSnapPre.exists()) {
    throw new Error('El gasto no existe.');
  }
  const expPre = expSnapPre.data() as Expense;
  const paidAmountPre = Number(expPre.paidAmount || (expPre.status === 'PAGADO' ? expPre.amount : 0));
  const wasPaidWithCashPre = (expPre.fundSource === 'CASH' || expPre.paymentMethod === 'EFECTIVO') && expPre.fundSource !== 'PERSONAL' && paidAmountPre > 0;

  let activeSession: any = null;
  if (wasPaidWithCashPre) {
    activeSession = await getCurrentSession(expPre.businessId);
    if (!activeSession || activeSession.status !== 'OPEN') {
      throw new Error('NO HAY UNA SESIÓN DE CAJA ABIERTA PARA REGISTRAR EL MOVIMIENTO.');
    }
  }
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (wasPaidWithCash && activeSession) {
      const movementDocId = getDeterministicCashMovementId(exp.businessId, 'EXPENSE_CANCELLATION', `cancel_exp_${cleanExpenseId}`);
      const cashMovRef = doc(db, 'cash_movements', movementDocId);
      transaction.set(cashMovRef, {
        id: cashMovRef.id,
        businessId: exp.businessId,
        ...
        sourceId: `cancel_exp_${cleanExpenseId}`,
        referenceType: 'EXPENSE',
        referenceId: cleanExpenseId,
        expenseId: cleanExpenseId,
        supplierName: exp.supplierName || 'Gasto Operativo',
...
export async function syncExpenseOperationToFirestore(op: OutboxOperation): Promise<{
  ...
  try {
    ...
    let activeSession: any = null;
    if (isPaidWithCash) {
      activeSession = await getCurrentSession(op.businessId);
      if (!activeSession || activeSession.status !== 'OPEN') {
        throw new Error('NO HAY UNA SESIÓN DE CAJA ABIERTA PARA REGISTRAR EL MOVIMIENTO.');
      }
    }
    ...
    await runTransaction(db, async (transaction) => {
      // 1. If paid immediately with business cash, create CashMovement atomically
      if (isPaidWithCash && activeSession) {
        const movementDocId = getDeterministicCashMovementId(op.businessId, 'EXPENSE', expenseId);
        const cashMovRef = doc(db, 'cash_movements', movementDocId);
        cashMovementId = movementDocId;
        transaction.set(cashMovRef, {
          id: cashMovementId,
      ...
      // 2. If PENDIENTE, create PaymentObligation atomically
      ...
      transaction.set(expenseRef, cleanPayload, { merge: true });
    });
