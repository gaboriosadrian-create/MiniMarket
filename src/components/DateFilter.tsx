...
export const DateFilter: React.FC<DateFilterProps> = ({ onDateRangeChange, viewTitle, lockToday = false }) => {
  ...
  const calculateRange = (selectedPreset: DatePreset, startCustomStr?: string, endCustomStr?: string) => {
    ...
    if (lockToday || selectedPreset === 'HOY') {
      start.setHours(0, 0, 0, 0);
      end.setHours(23, 59, 59, 999);
      label = `Hoy — ${todayDateFormatted}`;
  ...
  useEffect(() => {
    calculateRange(lockToday ? 'HOY' : preset, customStart, customEnd);
  }, [preset, lockToday]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs sm:text-sm font-extrabold tracking-tight text-stone-900">
            {viewTitle ? `${viewTitle} - ` : ''}
            <span className="text-stone-600 uppercase text-[11px] font-bold">Período de Consulta</span>
          </span>
        </div>

        {/* If lockToday is enabled, only display fixed Today badge without selectors */}
        {lockToday ? (
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-stone-900 text-white text-xs font-bold shadow-2xs select-none">
            <Clock className="w-3.5 h-3.5 text-emerald-400" />
            <span>Hoy — {todayDateFormatted}</span>
          </div>
        ) : (
          /* Presets buttons */
