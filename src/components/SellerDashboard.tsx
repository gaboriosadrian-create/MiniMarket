...
export const SellerDashboard: React.FC = () => {
  ...
  const [sellerCajaSubTab, setSellerCajaSubTab] = useState<'centro-caja' | 'resumen-dia'>('centro-caja');
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full">
          ...
          canCreateSales ? (
            <PosCaja
              onNavigateToCashControl={() => {
                setActiveTab('daily-control');
                setSellerCajaSubTab('centro-caja');
              }}
            />
          ) : (
        ...
        {/* VIEW 6: CONTROL DE CAJA Y CENTRO OPERATIVO */}
          ...
          canControlCaja ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2 bg-stone-100 p-1.5 rounded-2xl w-fit">
                <button
                  onClick={() => setSellerCajaSubTab('centro-caja')}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-2 cursor-pointer ${
                    sellerCajaSubTab === 'centro-caja'
                      ? 'bg-white text-stone-900 shadow-xs'
                      : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
                  <Coins className="w-4 h-4 text-amber-500" />
                  <span>Centro de Caja (Turno Actual y Arqueo)</span>
                </button>
                <button
                  onClick={() => setSellerCajaSubTab('resumen-dia')}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-2 cursor-pointer ${
                    sellerCajaSubTab === 'resumen-dia'
                      ? 'bg-white text-stone-900 shadow-xs'
                      : 'text-stone-600 hover:text-stone-900'
                  }`}
                >
