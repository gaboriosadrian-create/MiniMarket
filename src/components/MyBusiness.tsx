...
export const MyBusiness: React.FC = () => {
  ...
  // Subtab navigation: 'commercial' | 'appearance'
  const [activeSubTab, setActiveSubTab] = useState<'commercial' | 'appearance'>(() => {
    if (typeof window !== 'undefined') {
      const hash = window.location.hash.toLowerCase();
      if (hash === '#personalizacion' || hash === '#apariencia') {
        return 'appearance';
      }
    }
    return 'commercial';
  ...
  // Fetch Mercado Pago connection status (strictly requires business.id)
  const fetchMpStatus = async (targetBusinessId?: string) => {
    const bId = targetBusinessId || business?.id;
    if (!bId) {
      // Never query with 'default' when business is not yet available
      return null;
    }

    try {
      setMpLoading(true);
      const res = await fetch(getApiUrl(`/api/mercadopago/status?businessId=${encodeURIComponent(bId)}`));
      if (res.ok) {
        const data = await res.json();
        const isConn = Boolean(data.connected && data.connectionStatus === 'CONNECTED');
        const isEn = Boolean(data.enabled && isConn);
        const nextStatus = {
          enabled: isEn,
          connected: isConn,
          connectionStatus: isConn ? 'CONNECTED' : (data.connectionStatus || 'DISCONNECTED'),
          provider: data.provider || 'mercadopago',
          accountInfo: isConn ? data.accountInfo : null,
          lastError: data.lastError,
        };
        setMpStatus(nextStatus);
        return nextStatus;
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    const checkStatusAndParams = async () => {
      ...
      // Fetch real status for current businessId
      const statusResult = await fetchMpStatus(business.id);
      ...
      if (mpStatusParam === 'connected' || mpConnectedParam === 'true') {
        if (statusResult?.connected === true) {
          ...
          setMpFeedback({
            type: 'error',
            message: 'Mercado Pago completó la autorización, pero no se pudo confirmar la conexión activa de la cuenta. Por favor, reintenta la vinculación.',
          });
        }
        if (typeof window !== 'undefined') {
  ...
  const handleConnectMercadoPago = async () => {
    ...
    try {
      ...
      const returnOriginParam = typeof window !== 'undefined' ? `&returnOrigin=${encodeURIComponent(window.location.origin)}` : '';
      const connectUrl = getApiUrl(`/api/mercadopago/connect?businessId=${encodeURIComponent(business.id)}${uidParam}${returnOriginParam}`);
  ...
  const handleConfirmDisconnect = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('/api/mercadopago/disconnect'), {
        method: 'POST',
  ...
  const isMpConnected = Boolean(mpStatus?.connected && mpStatus?.connectionStatus === 'CONNECTED' && mpStatus?.accountInfo);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {isAdmin && !isEditing && activeSubTab === 'commercial' && (
          <button
            type="button"
      ...
      {/* SUBTAB NAVIGATION: Información Comercial vs Personalización */}
      <div className="flex items-center gap-2 border-b border-stone-200">
        <button
          type="button"
          onClick={() => {
            setActiveSubTab('commercial');
            if (typeof window !== 'undefined') window.location.hash = '#minegocio';
          }}
        ...
        >
          ...
          <span>Información Comercial y Cobros</span>
        </button>
        ...
        <button
          ...
          onClick={() => {
            ...
            if (typeof window !== 'undefined') window.location.hash = '#personalizacion';
          }}
          className={`px-4 py-2.5 text-xs font-bold border-b-2 transition-all flex items-center gap-2 cursor-pointer ${
