...
export const MyBusiness: React.FC = () => {
  ...
  const fetchMpStatus = async (targetBusinessId?: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const isEn = Boolean(data.enabled ?? data.config?.enabled);
        const nextStatus = {
          enabled: isEn,
          connected: isConn,
          connectionStatus: data.connectionStatus || (isConn ? 'CONNECTED' : (isEn ? 'NOT_VERIFIED' : 'DISCONNECTED')),
  ...
  const isMpConnected = Boolean(mpStatus?.connected);
