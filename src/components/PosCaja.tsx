...
import { Product, CartItem, PosStatus, PaymentMethod, Sale, PosNote } from '../types';
...
export const PosCaja: React.FC = () => {
  ...
  const frequentProducts = useMemo(() => {
    return products.filter((p) => p.active).slice(0, 16);
  }, [products]);
  ...
  const handleConfirmSale = async () => {
    ...
    if (paymentMethod === 'EFECTIVO') {
      ...
      if (cashPart + mpPart !== totalAmount) {
    ...
    try {
      ...
      // Trigger Thermal Receipt Printing Animation with real immutable sale data
      setLastCompletedSale(saleResult);
      setShowReceiptAnimation(true);

      if (saleResult.syncStatus === 'PENDING') {
        showNotification(
          `Venta #${saleResult.id?.slice(-6).toUpperCase()} guardada en Modo Offline. Se sincronizará automáticamente al volver la conexión.`,
          'info'
        );
      }
      setCart([]);
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:items-stretch">
        ...
        <div className="lg:col-span-7 space-y-3">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-2.5 sm:p-4 shadow-2xs space-y-2 sm:space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-amber-50 border border-amber-200/80 flex items-center justify-center text-amber-600 shadow-2xs">
                  <Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-amber-500 text-amber-500" />
                </div>
                ...
                {frequentProducts.map((p) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative bg-stone-50/90 hover:bg-white active:bg-stone-100/90 rounded-xl border border-stone-200/90 hover:border-stone-300 shadow-2xs hover:shadow-xs transition-all duration-150 flex flex-col justify-between p-1.5 sm:p-2.5 cursor-pointer select-none active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none min-w-0 h-full ${
                        isInactive ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''
                      }`}
                    >
                      {/* Top Row: Category/Product Icon & '+' Button */}
                      <div className="flex items-center justify-between gap-1 w-full">
                        <div
                          className={`w-6 h-6 sm:w-7.5 sm:h-7.5 rounded-lg ${colorTheme.bg} ${colorTheme.border} ${colorTheme.text} border flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-105 transition-transform overflow-hidden`}
                        >
                          {hasImage ? (
                            <img
                              src={imgSrc}
                        ...
                        <button
                          ...
                          className="w-5 h-5 sm:w-6 sm:h-6 rounded-md bg-stone-900 hover:bg-indigo-600 active:bg-indigo-700 text-white flex items-center justify-center transition-all duration-150 cursor-pointer shadow-2xs active:scale-90 hover:scale-105 shrink-0 focus:outline-none"
                        >
                          <Plus className="w-3 h-3 sm:w-3.5 sm:h-3.5 stroke-[2.5]" />
                        </button>
                      </div>

                      {/* Middle: Product Name (Fixed 2-line container for uniform grid height) */}
                      <div className="my-1 sm:my-1.5 min-w-0 w-full min-h-[24px] sm:min-h-[28px] flex items-center">
                        <p
                          className="font-bold text-[10px] sm:text-xs text-stone-900 line-clamp-2 leading-tight group-hover:text-indigo-950 transition-colors break-words"
                          title={p.name}
                        >
                          {p.name}
                        </p>
                      ...
                      {/* Bottom: Price & Stock */}
                      <div className="flex items-baseline justify-between gap-1 w-full pt-1 border-t border-stone-200/60 min-w-0 mt-auto">
                        <span className="font-bold text-[10px] sm:text-xs text-stone-900 font-mono tracking-tight group-hover:text-indigo-900 transition-colors truncate">
                          {formatCurrency(p.salePrice)}
                        </span>
                        <span className="text-[8px] sm:text-[9px] text-stone-400 font-mono shrink-0">
                          {p.stock}u
                        </span>
                      </div>
                    </div>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/50 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white rounded max-w-md w-full p-5 sm:p-6 shadow-xl border border-stone-200 space-y-4 animate-in fade-in zoom-in duration-150">
            
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-stone-100 pb-2.5">
              <div className="flex items-center space-x-2">
                <div className="w-8 h-8 rounded bg-blue-50 text-[#006AFF] border border-blue-200 flex items-center justify-center font-bold">
            ...
            {/* Total Display */}
            <div className="bg-stone-900 text-white p-3.5 rounded text-center space-y-0.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Total A Cobrar</span>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSplitMpAmount(totalAmount - val);
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSplitCashReceived(String(totalAmount - val));
                        setSaleError(null);
                      }}
                      onFocus={(e) => e.target.select()}
                      onClick={(e) => (e.target as HTMLInputElement).select()}
            ...
            <div className="flex gap-2 pt-2 border-t border-stone-100">
              <button
                ...
                className="w-1/3 py-2.5 bg-stone-100 hover:bg-stone-200 disabled:opacity-50 text-stone-800 font-bold rounded text-xs transition-colors cursor-pointer border border-stone-200"
              >
                Volver
              </button>
              ...
              <button
                ...
                disabled={
                  ...
                  (paymentMethod === 'COMBINADO' &&
                    (splitCashAmount + splitMpAmount !== totalAmount ||
                     isNaN(Number(splitCashReceived)) || Number(splitCashReceived) < splitCashAmount))
                }
                id="btn-confirm-sale"
                className="w-2/3 py-2.5 bg-[#006AFF] hover:bg-[#0052CC] disabled:bg-stone-200 disabled:text-stone-400 text-white font-bold rounded text-xs sm:text-sm transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer active:scale-98"
              >
                {submittingSale ? (
                  <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 backdrop-blur-xs p-3 sm:p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-4 sm:p-5 shadow-2xl space-y-3.5 border border-stone-200 text-stone-900 max-h-[90vh] flex flex-col">
            ...
            <div className="grid grid-cols-3 gap-2 shrink-0">
              <div className="bg-stone-50 p-2.5 rounded-2xl border border-stone-200 text-center">
                <span className="text-[10px] uppercase font-bold text-stone-500 block">Total Turno</span>
                <span className="text-base font-black text-stone-900 font-mono">{todaySales.length}</span>
              </div>

              <div className="bg-emerald-50 p-2.5 rounded-2xl border border-emerald-200 text-center">
                <span className="text-[10px] uppercase font-bold text-emerald-800 block">Total Facturado</span>
                <span className="text-base font-black text-emerald-800 font-mono">
                  {formatCurrency(todaySales.reduce((acc, s) => acc + (s.total || 0), 0))}
                </span>
              </div>

              <div className="bg-blue-50 p-2.5 rounded-2xl border border-blue-200 text-center">
                <span className="text-[10px] uppercase font-bold text-blue-800 block">Efectivo / MP</span>
                <span className="text-[11px] font-black text-blue-900 font-mono block pt-0.5 truncate">
                  💵 {formatCurrency(todaySales.reduce((sum, s) => {
            ...
            {/* QUICK FILTERS ACCORDION / PANEL */}
            <div className="bg-stone-50/80 p-3 rounded-2xl border border-stone-200 space-y-2.5 shrink-0 text-xs">
              ...
              {/* Medio de Pago Pills & Result Count */}
              <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5 border-t border-stone-200/60">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold text-stone-600">
                    <strong className="text-stone-900">{filteredTodaySales.length}</strong> {filteredTodaySales.length === 1 ? 'venta encontrada' : 'ventas encontradas'}
                  </span>
                  {hasHistoryFilters && (
                    <button
                      type="button"
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2.5 shrink-0">
                          ...
                          <div className="p-1 rounded-lg bg-white border border-stone-200 text-stone-500">
                      ...
                      {isExpanded && (
                        <div className="px-3.5 pb-3.5 pt-1 border-t border-stone-200/80 bg-white space-y-2.5 animate-in fade-in">
                          ...
                          <div className="bg-stone-50 p-2.5 rounded-xl border border-stone-200 space-y-1.5 text-xs">
                            ...
                            {sale.paymentMethod === 'MERCADO_PAGO' && (
                              <div className="pt-1 border-t border-stone-200/60 flex justify-between text-[11px] text-stone-600">
                                <span>Medio de Pago:</span>
                                <strong className="text-sky-700">Mercado Pago / Transferencia (${sale.total})</strong>
                              </div>
                            )}
      ...
      {/* THERMAL RECEIPT PRINT ANIMATION (REAL SALE DATA) */}
