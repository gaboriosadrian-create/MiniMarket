...
import { Product, CartItem, PosStatus, PaymentMethod, Sale, PosNote, MercadoPagoSource, CashSession } from '../types';
...
import { getCurrentSession, getOrCreateDefaultCashRegister } from '../lib/cashCoreService';
...
export interface PosCajaProps {
  onNavigateToCashControl?: () => void;
}

export const PosCaja: React.FC<PosCajaProps> = ({ onNavigateToCashControl }) => {
  const { user, userProfile, business } = useAuth();
  ...
  const [checkingCashSession, setCheckingCashSession] = useState<boolean>(false);
  const [showCashClosedNotice, setShowCashClosedNotice] = useState<boolean>(false);
  ...
  const loadMpConfig = async () => {
    if (!business?.id) {
      // Do not query when business.id is not yet available
      return;
    }
    try {
      const res = await fetch(getApiUrl(`/api/mercadopago/status?businessId=${encodeURIComponent(business.id)}`));
      if (res.ok) {
        const data = await res.json();
        const isEnabled = Boolean(data.enabled ?? data.config?.enabled);
        const isConnected = Boolean(data.connected ?? (data.connectionStatus === 'CONNECTED'));
        const connStatus = data.connectionStatus || (isConnected ? 'CONNECTED' : (isEnabled ? 'NOT_VERIFIED' : 'DISCONNECTED'));
        const mode = data.config?.mode || 'TEST';
  ...
  // Load active cash session for current business
  const loadCashSession = async () => {
    if (!business?.id) return;
    setCheckingCashSession(true);
    try {
      const defaultReg = await getOrCreateDefaultCashRegister(business.id, undefined, userProfile?.uid, userProfile?.displayName);
      const session = await getCurrentSession(business.id, defaultReg.id);
      setCurrentCashSession(session);
    } catch (err) {
      console.error('[PosCaja] Error loading cash session:', err);
    } finally {
      setCheckingCashSession(false);
    }
  };

  // 1. Restore draft cart from sessionStorage on mount
  useEffect(() => {
    if (!business?.id) return;
    try {
      const saved = sessionStorage.getItem(`uwi_pos_draft_cart_${business.id}`);
      if (saved) {
      ...
      console.error('Error reading draft cart from sessionStorage:', e);
    }
  }, [business?.id]);

  // 2. Persist draft cart to sessionStorage whenever cart changes
  useEffect(() => {
    if (!business?.id) return;
    try {
      if (cart.length > 0) {
        sessionStorage.setItem(`uwi_pos_draft_cart_${business.id}`, JSON.stringify(cart));
      } else {
        sessionStorage.removeItem(`uwi_pos_draft_cart_${business.id}`);
      }
    } catch (e) {
      // ignore
    }
  ...
  const cancelActiveMpOrder = async () => {
    ...
    if (mpOrderRef && (mpOrderState === 'WAITING_PAYMENT' || mpOrderState === 'CREATING')) {
      try {
        ...
        await fetch(getApiUrl('/api/mercadopago/cancel-order'), {
          method: 'POST',
          headers,
          body: JSON.stringify({
  ...
  const createMpOrderForPos = async (amount: number, isSplit = false, sourceOverride?: MercadoPagoSource) => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('/api/mercadopago/create-order'), {
        method: 'POST',
        headers,
        body: JSON.stringify({
          businessId: business?.id,
  ...
  const handleSplitCashInputChange = (rawVal: string) => {
    ...
    // Cancel active MP order in background to stop Point/MP terminal and clear previous pending order
    cancelActiveMpOrder();

    // Invalidate local state of previous QR and stop polling immediately
    setMpOrderRef(null);
    setMpOrderId(null);
    setMpPaymentId(null);
    setMpOrderAmount(null);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const orderStatusUrl = getApiUrl(`/api/mercadopago/order-status?external_reference=${encodeURIComponent(mpOrderRef)}${businessParam}`);
  ...
  const handleConfirmSale = async (opts?: { autoVerified?: boolean }) => {
    ...
    // 0. Physical Cash Session Validation:
    // If sale involves cash, a CashSession with status OPEN must exist
    const cashPortion = paymentMethod === 'EFECTIVO' ? totalAmount : (paymentMethod === 'COMBINADO' ? (Number(splitCashAmount) || 0) : 0);
    if (cashPortion > 0) {
      let activeSession = currentCashSession;
      try {
        const defaultReg = await getOrCreateDefaultCashRegister(business.id, undefined, userProfile.uid, userProfile.displayName);
        activeSession = await getCurrentSession(business.id, defaultReg.id);
        setCurrentCashSession(activeSession);
      } catch (err) {
        console.error('[PosCaja] Error re-checking cash session:', err);
      }

      if (!activeSession || activeSession.status !== 'OPEN') {
        setShowCashClosedNotice(true);
        setSaleError('Para cobrar en efectivo necesitás abrir la caja primero.');
        return;
      }
    }
    ...
    if (paymentMethod === 'EFECTIVO') {
      ...
      if (isMpActive && mpPart > 0 && isOnlineCheck) {
        if (
          ...
          setSaleError('El importe de Mercado Pago cambió. Regenerá el QR antes de confirmar el cobro.');
          return;
        }
      }
      ...
      <div className="bg-white rounded p-3 sm:p-3.5 border border-stone-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
          ...
          <div className="hidden sm:flex items-center space-x-2 text-xs">
            ...
            {/* Discrete Cash Session Indicator */}
            {currentCashSession?.status === 'OPEN' ? (
              <button
                type="button"
                onClick={onNavigateToCashControl}
                className="px-2.5 py-1 rounded font-bold text-[11px] flex items-center gap-1.5 bg-emerald-50 text-emerald-800 border border-emerald-200 hover:bg-emerald-100 transition-colors cursor-pointer"
                title="Caja abierta · Clic para abrir Centro de Caja"
              >
              ...
              <button
                type="button"
                onClick={onNavigateToCashControl}
                className="px-2.5 py-1 rounded font-bold text-[11px] flex items-center gap-1.5 bg-stone-100 text-stone-600 border border-stone-200 hover:bg-stone-200 transition-colors cursor-pointer"
                title="Caja cerrada · Clic para abrir caja"
              >
          ...
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[90dvh] sm:max-h-[86vh] flex flex-col shadow-2xl border border-stone-200 overflow-hidden animate-in fade-in zoom-in duration-150">
            ...
            {paymentMethod === 'EFECTIVO' && (
              <div className="space-y-3 pt-1 border-t border-stone-100">
                ...
                {/* CLOSED CASH SESSION WARNING (Section 8 of Etapa 3B) */}
                {(!currentCashSession || currentCashSession.status !== 'OPEN') && (
                  <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-2xl text-stone-900 space-y-3 animate-in fade-in">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="space-y-0.5">
                        <h4 className="font-black text-xs sm:text-sm text-stone-900 uppercase tracking-wider">
                          CAJA CERRADA
                        </h4>
                        <p className="text-xs text-stone-700 leading-relaxed">
                          Esta venta requiere efectivo, pero la caja está cerrada.
                        </p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 pt-1">
                      {onNavigateToCashControl && (
                        <button
                          ...
                          id="btn-pos-abrir-caja"
                          className="flex-1 py-2.5 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl shadow-xs flex items-center justify-center space-x-1.5 cursor-pointer active:scale-95"
                        >
                      ...
                      <button
                        ...
                        id="btn-pos-cambiar-metodo"
                        className="flex-1 py-2.5 px-3 bg-white hover:bg-stone-100 text-stone-800 font-bold text-xs rounded-xl border border-stone-300 flex items-center justify-center space-x-1.5 cursor-pointer"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => setShowPaymentModal(false)}
                        id="btn-pos-cancelar-aviso"
                        className="py-2.5 px-3 bg-stone-100 hover:bg-stone-200 text-stone-600 font-bold text-xs rounded-xl cursor-pointer"
                      >
                        CANCELAR
                      </button>
                    </div>
            ...
            {paymentMethod === 'COMBINADO' && (
              <div className="space-y-3 pt-1 border-t border-purple-100 bg-purple-50/40 p-2.5 rounded-xl border">
                ...
                {/* CLOSED CASH SESSION WARNING FOR COMBINADO */}
                {splitCashAmount > 0 && (!currentCashSession || currentCashSession.status !== 'OPEN') && (
                  <div className="p-3.5 bg-amber-50 border border-amber-300 rounded-xl text-stone-900 space-y-2.5 animate-in fade-in">
                    <div className="flex items-start space-x-2.5">
                      ...
                      <div className="space-y-0.5">
                        <h4 className="font-black text-xs text-stone-900 uppercase tracking-wider">
                          CAJA CERRADA (Parte Efectivo)
                        </h4>
                        <p className="text-[11px] text-stone-700 leading-relaxed">
                          La parte en efectivo ({formatCurrency(splitCashAmount)}) requiere abrir la caja.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-1.5 pt-0.5">
                      {onNavigateToCashControl && (
                        <button
                          ...
                          className="flex-1 py-2 px-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-lg shadow-xs flex items-center justify-center space-x-1 cursor-pointer active:scale-95"
                        >
                      ...
                      <button
                        ...
                        className="flex-1 py-2 px-2.5 bg-white hover:bg-stone-100 text-stone-800 font-bold text-xs rounded-lg border border-stone-300 flex items-center justify-center space-x-1 cursor-pointer"
                      >
                        <QrCode className="w-3.5 h-3.5 text-sky-600" />
                        <span>COBRAR 100% CON MP</span>
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="text-[11px] pt-1 space-y-2">
                    {!isMpActive ? (
                      ...
                      <>
                        ...
                        {qrNeedsRegeneration ? (
                          <div className="p-3 bg-amber-50/95 border-2 border-amber-300 rounded-xl space-y-2.5 text-amber-950 shadow-2xs">
                            <div className="flex items-start gap-2.5">
                              <AlertCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                              <div className="space-y-0.5">
                                <p className="font-extrabold text-xs text-amber-950 flex items-center gap-1.5">
                                  <span>⚠️ Importe modificado</span>
                                </p>
                                <p className="text-xs text-amber-900 leading-snug">
                                  El importe de Mercado Pago cambió a <strong className="font-mono font-black text-amber-950">{formatCurrency(splitMpAmount)}</strong>.
                                </p>
                                <p className="text-[11px] font-medium text-amber-800">
                                  Regenerá el QR para continuar.
                                </p>
                              </div>
                            </div>
                            <button
                              ...
                              className="w-full py-2 px-3 bg-[#006AFF] hover:bg-[#0052CC] text-white rounded-lg text-xs font-black transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer active:scale-98"
                            >
                              <RefreshCw className="w-3.5 h-3.5" />
                              <span>Regenerar cobro</span>
                            </button>
                          </div>
                        ) : (
                          <>
            ...
            <div className="shrink-0 px-5 py-3 border-t border-stone-100 bg-stone-50/95 flex gap-2">
              ...
              >
                {submittingSale ? (
                  ...
                  isMpActive && splitMpAmount > 0 && (typeof navigator === 'undefined' || navigator.onLine) ? (
                    (qrNeedsRegeneration || mpOrderAmount === null || Math.abs(mpOrderAmount - splitMpAmount) > 0.01 || mpOrderState === 'IDLE' || mpOrderState === 'ERROR') ? (
                      <>
                        <AlertCircle className="w-4 h-4" />
                        <span>Regenerá el QR para continuar</span>
                      </>
                    ) : (mpOrderState === 'PAYMENT_VERIFIED' || mpOrderState === 'CONFIRMED') ? (
                      <>
                        <Check className="w-4 h-4" />
