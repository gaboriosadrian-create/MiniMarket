...
export const ReceiptPrintAnimation: React.FC<ReceiptPrintAnimationProps> = ({
  ...
  }, [durationMs, prefersReducedMotion, isDismissed, autoDismiss]);
  ...
  return (
    <div
      ...
      aria-modal="false"
      className={`fixed inset-0 z-[9999] pointer-events-none flex flex-col items-center justify-start pt-12 sm:pt-16 px-4 transition-opacity duration-300 ${
          ...
          >
            ...
            >
              ...
              <div className="border-t border-dashed border-stone-300 pt-2 space-y-1">
                ...
                <div className="border-t border-stone-200 pt-1.5 space-y-1 text-[10px] text-stone-700">
                  {sale.paymentMethod === 'EFECTIVO' && (
                    <div className="space-y-0.5">
                      <div className="flex justify-between font-semibold">
                        <span>Medio de Pago:</span>
                        <span className="font-bold text-stone-900 uppercase">EFECTIVO</span>
                      </div>
                      {sale.cashReceived !== undefined && sale.cashReceived > 0 && (
                        <div className="flex justify-between text-stone-600 text-[9.5px]">
                          <span>Efectivo Recibido:</span>
                          <span className="font-mono">${sale.cashReceived.toLocaleString('es-AR')}</span>
                        </div>
                      )}
                      {sale.change !== undefined && sale.change > 0 && (
                        <div className="flex justify-between font-bold text-emerald-800 text-[9.5px]">
                  ...
                  {sale.paymentMethod === 'MERCADO_PAGO' && (() => {
                    const isMpOffline = Boolean(sale.syncMode === 'OFFLINE' || sale.offline || sale.paymentDetails?.mode === 'OFFLINE');
                    const opId = sale.paymentDetails?.paymentId || (sale.paymentDetails as any)?.operationId || (sale.paymentDetails?.externalReference && !sale.paymentDetails.externalReference.startsWith('MINIMARKET-') ? sale.paymentDetails.externalReference : null);
                    const ordId = sale.paymentDetails?.orderId;

                    return (
                      <div className="space-y-1">
                        <div className="flex justify-between font-bold text-stone-900 uppercase text-[9.5px]">
                          <span>MEDIO DE PAGO</span>
                        </div>
                        <div className="font-black text-[#006AFF] text-xs">
                          Mercado Pago
                        </div>

                        {isMpOffline ? (
                          <div className="space-y-0.5 pt-0.5 text-[9.5px] text-stone-700">
                            <div className="flex justify-between">
                              <span>Importe:</span>
                              <span className="font-mono font-bold text-stone-900">${sale.total.toLocaleString('es-AR')}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Estado:</span>
                              <span className="font-bold text-amber-800">Registrado offline</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Verificación:</span>
                              <span className="font-semibold text-stone-700">Manual</span>
                            </div>
                          </div>
                        ) : (
                          <div className="space-y-0.5 pt-0.5 text-[9.5px] text-stone-700">
                            {sale.paymentDetails?.mercadoPagoSource && (
                              <div className="flex justify-between">
                                <span>Modalidad:</span>
                                <span className="font-bold text-stone-900">
                                  {sale.paymentDetails.mercadoPagoSource === 'POINT_GENERATED_QR' ? 'QR Point/POS' : 'QR Físico'}
                                </span>
                              </div>
                            )}
                            {opId && (
                              <div className="flex justify-between">
                                <span>Operación:</span>
                                <span className="font-mono font-bold text-stone-900">#{String(opId).replace(/^#+/, '').trim()}</span>
                              </div>
                            )}
                            {ordId && (
                              <div className="flex justify-between">
                                <span>Orden:</span>
                                <span className="font-mono text-stone-800 text-[8.5px] truncate max-w-[170px] text-right" title={ordId}>{ordId}</span>
                              </div>
                            )}
                            <div className="flex justify-between">
                              <span>Importe:</span>
                            ...
                            <div className="flex justify-between">
                              <span>Estado:</span>
                              <span className="font-bold text-emerald-800">Acreditado</span>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  {sale.paymentMethod === 'COMBINADO' && (() => {
                    ...
                    const opId = sale.paymentDetails?.paymentId || (sale.paymentDetails as any)?.operationId || (sale.paymentDetails?.externalReference && !sale.paymentDetails.externalReference.startsWith('MINIMARKET-') ? sale.paymentDetails.externalReference : null);
                    const ordId = sale.paymentDetails?.orderId;
                    const cashAmt = sale.paymentBreakdown?.cashAmount || 0;
                    const mpAmt = sale.paymentBreakdown?.mpAmount || 0;

                    return (
                      <div className="space-y-1">
                        <div className="font-bold text-stone-900 uppercase text-[9.5px]">
                          MEDIOS DE PAGO
                        </div>
                        <div className="space-y-0.5 text-[9.5px] text-stone-700">
                          <div className="flex justify-between">
                            <span>Efectivo:</span>
                            <span className="font-mono font-bold text-stone-900">${cashAmt.toLocaleString('es-AR')}</span>
                          </div>
                          {sale.paymentBreakdown?.cashReceived !== undefined && sale.paymentBreakdown.cashReceived > cashAmt && (
                            <div className="flex justify-between text-stone-500 text-[8.5px] pl-2">
                              <span>Recibido: ${sale.paymentBreakdown.cashReceived.toLocaleString('es-AR')}</span>
                              <span>Vuelto: ${(sale.paymentBreakdown.change || 0).toLocaleString('es-AR')}</span>
                            </div>
                          )}
                          <div className="flex justify-between">
                            <span>Mercado Pago:</span>
                            <span className="font-mono font-bold text-stone-900">${mpAmt.toLocaleString('es-AR')}</span>
                          </div>
                        </div>

                        {mpAmt > 0 && (
                          <div className="mt-1 pt-1 border-t border-dashed border-stone-200 space-y-0.5 text-[9px] text-stone-700">
                            <div className="font-bold text-[#006AFF] text-[9.5px]">Mercado Pago</div>
                            {isMpOffline ? (
                              <>
                                <div className="flex justify-between">
                                  <span>Importe:</span>
                              ...
                              <>
                                ...
                                <div className="flex justify-between">
                                  <span>Estado:</span>
                                  <span className="font-bold text-emerald-800">Pago verificado</span>
                                </div>
                              </>
                            )}
                          </div>
