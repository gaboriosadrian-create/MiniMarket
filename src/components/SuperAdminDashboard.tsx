...
import { BusinessAdminsManagementModal } from './BusinessAdminsManagementModal';
import { 
  Building2, 
  Plus, 
  Search, 
...
export const SuperAdminDashboard: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'businesses' | 'design-lab' | 'technical-config'>('businesses');
  const [targetConfigBusinessId, setTargetConfigBusinessId] = useState<string | undefined>(undefined);
  ...
  const [managingInitialTab, setManagingInitialTab] = useState<'business' | 'admins'>('admins');
    ...
    setNavItems([
      {
        ...
        isActive: activeView === 'businesses' && statusFilter === 'all',
        onClick: () => {
      ...
      {
        id: 'technical-config',
        label: '⚙️ Configuración Técnica MP',
        icon: <Settings className="w-4 h-4 text-purple-500" />,
        isActive: activeView === 'technical-config',
        onClick: () => setActiveView('technical-config'),
      },
      {
        id: 'design-lab',
        label: '🎨 Design Lab',
        icon: <Palette className="w-4 h-4 text-pink-500" />,
        isActive: activeView === 'design-lab',
        onClick: () => setActiveView('design-lab'),
      },
      {
        id: 'create-business',
      ...
      },
      {
        ...
        isActive: activeView === 'businesses' && statusFilter === 'active',
        onClick: () => {
          setActiveView('businesses');
      ...
      {
        id: 'inactive-businesses',
        label: '⛔ Negocios Inactivos',
        icon: <XCircle className="w-4 h-4 text-red-500" />,
        isActive: activeView === 'businesses' && statusFilter === 'inactive',
        onClick: () => {
          setActiveView('businesses');
  ...
  }, [activeView, statusFilter, setNavItems]);
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBusinessWithAdmin(
        formData, 
        userProfile ? { uid: userProfile.uid, email: userProfile.email } : undefined
      );
      setFormData({
        businessName: '',
        adminName: '',
  ...
  // Technical Config View (Mercado Pago Multi-Tenant)
  if (activeView === 'technical-config') {
    return (
      <div className="min-h-screen bg-stone-50">
        <div className="bg-stone-900 text-white px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between border-b border-stone-800">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1 text-xs font-bold rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-200 cursor-pointer transition-colors"
              >
                🏢 Gestión de Negocios
              </button>
              <button
                type="button"
                onClick={() => setActiveView('technical-config')}
                className="px-3 py-1 text-xs font-bold rounded-lg bg-purple-600 text-white cursor-pointer shadow-xs flex items-center gap-1"
              >
                <Settings className="w-3.5 h-3.5" />
                <span>⚙️ Configuración Técnica MP (Activo)</span>
              </button>
              <button
                type="button"
                onClick={() => setActiveView('design-lab')}
                className="px-3 py-1 text-xs font-bold rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-200 cursor-pointer transition-colors flex items-center gap-1"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <AdminSettings initialBusinessId={targetConfigBusinessId} />
        </div>
      </div>
    );
  }

  if (activeView === 'design-lab') {
    return (
      <div className="space-y-4">
        {/* Top bar to switch back to Super Admin businesses if desired */}
        <div className="bg-stone-900 text-white px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between border-b border-stone-800">
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold uppercase tracking-wider text-purple-300">
              Super Admin View:
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setActiveView('design-lab')}
                className="px-3 py-1 text-xs font-bold rounded-lg bg-emerald-600 text-white cursor-pointer shadow-xs"
              >
                🎨 Design Lab (Activo)
              </button>
            </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {/* Top Switcher & Banner */}
      <div className="flex items-center justify-between bg-stone-900/90 text-white p-3 rounded-2xl border border-stone-800 text-xs">
        <div className="flex items-center gap-2">
          <span className="font-bold text-stone-400">Modo:</span>
          <button
            type="button"
            onClick={() => setActiveView('businesses')}
            className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors ${
              activeView === 'businesses' ? 'bg-purple-600 text-white' : 'bg-stone-800 hover:bg-stone-700 text-stone-300'
            }`}
          >
            🏢 Gestión de Negocios
          </button>
          <button
            type="button"
            onClick={() => setActiveView('technical-config')}
            className={`px-3 py-1 rounded-lg font-bold cursor-pointer transition-colors flex items-center gap-1 ${
              activeView === 'technical-config' ? 'bg-purple-600 text-white' : 'bg-stone-800 hover:bg-stone-700 text-stone-300'
            }`}
          >
            <Settings className="w-3.5 h-3.5 text-stone-300" />
            <span>⚙️ Configuración Técnica MP</span>
          </button>
          <button
            type="button"
            onClick={() => setActiveView('design-lab')}
            className="px-3 py-1 rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-300 font-bold cursor-pointer transition-colors flex items-center gap-1"
          >
            <Palette className="w-3.5 h-3.5 text-pink-400" />
            <span>🎨 Design Lab</span>
          </button>
      ...
      <div className="bg-gradient-to-r from-purple-900 to-indigo-900 rounded-2xl p-6 sm:p-8 text-white shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-purple-100 text-sm mt-1 max-w-2xl">
            Crea, activa o administra todos los comercios y negocios en la plataforma.
          </p>
        </div>
            ...
            <tbody className="bg-white divide-y divide-stone-200">
              {loading ? (
                ...
                filteredBusinesses.map((biz) => {
                  const adminCount = Array.isArray(biz.adminUserIds) ? biz.adminUserIds.length : (biz.adminUserId ? 1 : 0);
                  ...
                  return (
                    <tr key={biz.id} className="hover:bg-stone-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div>
                            <div className="text-sm font-medium text-stone-800">{biz.adminName || 'Admin principal'}</div>
                            <div className="text-xs text-stone-500 font-mono">{biz.adminEmail || '-'}</div>
                          </div>
                          {adminCount > 1 && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black bg-purple-100 text-purple-800 border border-purple-200">
                              +{adminCount - 1} admin{adminCount - 1 > 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <button
                          onClick={() => {
                            ...
                            setManagingInitialTab('admins');
                          }}
                          className="inline-flex items-center px-2.5 py-1.5 text-xs font-bold rounded-lg bg-purple-700 hover:bg-purple-800 text-white shadow-xs transition-all cursor-pointer"
                          title="Gestionar Administradores y Datos del Comercio"
                        >
                          <Users className="w-3.5 h-3.5 mr-1" /> Editar & Admins
                        </button>

                        <button
                          onClick={() => {
                            setTargetConfigBusinessId(biz.id);
                            setActiveView('technical-config');
                          }}
                          className="inline-flex items-center px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-stone-100 text-stone-700 hover:bg-stone-200 border border-stone-200 transition-colors cursor-pointer"
                          title="Configurar pasarela Mercado Pago"
                        >
                          <Settings className="w-3.5 h-3.5 mr-1" /> Config MP
                        </button>

                        <button
                          onClick={() => handleToggleStatus(biz)}
                          className={`inline-flex items-center px-2.5 py-1.5 text-xs font-semibold rounded-lg transition-colors cursor-pointer ${
                            biz.status === 'active'
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-stone-200 space-y-5 animate-in fade-in zoom-in duration-150">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-stone-700 mb-1">
                  Gmail / Correo del Administrador *
                </label>
                <div className="relative">
              ...
              <div className="rounded-xl bg-blue-50/80 border border-blue-200/80 p-3.5 flex items-start space-x-2.5 text-xs text-blue-900">
                <ShieldCheck className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                <div className="space-y-0.5">
                  <p className="font-bold text-blue-950">
                    Acceso delegado con Google
                  </p>
                  <p className="text-blue-800 text-[11px] leading-relaxed">
                    El administrador ingresará a Uwi utilizando su cuenta de Google. No es necesario establecer una contraseña.
                  </p>
                </div>
              </div>
      ...
      {/* MODAL: Gestionar Administradores y Datos del Negocio (1:N) */}
      {managingBusiness && (
        <BusinessAdminsManagementModal
          business={managingBusiness}
          superAdminUser={{
            uid: authUser?.uid || userProfile?.id || 'superadmin',
            email: authUser?.email || userProfile?.email || 'superadmin@uwi.com',
          }}
          initialTab={managingInitialTab}
          onClose={() => setManagingBusiness(null)}
          onBusinessUpdated={(updated) => {
            setBusinesses((prev) => prev.map((b) => (b.id === updated.id ? updated : b)));
            setManagingBusiness(updated);
          }}
        />
      )}
