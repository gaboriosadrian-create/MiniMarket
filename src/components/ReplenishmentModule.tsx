...
import { 
  ...
  submitReplenishmentForApproval
} from '../lib/replenishmentService';
import { updateBusinessSettings } from '../lib/businessService';
...
import { ReplenishmentHistoryView } from './ReplenishmentHistoryView';
...
export const ReplenishmentModule: React.FC = () => {
  ...
  // Sub-tabs in OVERVIEW: 'SOLICITUD' | 'SUGGESTIONS' | 'APPROVALS' | 'HISTORY'
  const [activeTab, setActiveTab] = useState<'SOLICITUD' | 'SUGGESTIONS' | 'APPROVALS' | 'HISTORY'>('SOLICITUD');
  ...
  // Approvals & History state
  const [pendingApprovals, setPendingApprovals] = useState<ReplenishmentList[]>([]);
  const [loadingApprovals, setLoadingApprovals] = useState(true);
  const [historyOrders, setHistoryOrders] = useState<ReplenishmentList[]>([]);
  const [loadingHistory, setLoadingHistory] = useState(true);
  const [selectedApprovalOrder, setSelectedApprovalOrder] = useState<ReplenishmentList | null>(null);
  const [updatingApprovalSetting, setUpdatingApprovalSetting] = useState(false);
  const [focusedOrderId, setFocusedOrderId] = useState<string | null>(null);

  // Approval setting (default is true if undefined)
  const approvalRequired = business?.settings?.replenishmentApprovalRequired ?? true;
  ...
  // Real-time Firestore subscriptions for pending approvals and non-draft history orders
  useEffect(() => {
    if (!business?.id) return;

    // Listen for PENDING_APPROVAL lists
    ...
    const unsubscribePending = onSnapshot(qPending, (snapshot) => {
      ...
      snapshot.forEach(docSnap => {
        list.push({ id: docSnap.id, ...docSnap.data() } as ReplenishmentList);
      });
      list.sort((a, b) => (b.submittedAt || b.createdAt || '').localeCompare(a.submittedAt || a.createdAt || ''));
      setPendingApprovals(list);
      setLoadingApprovals(false);
    }, (err) => {
      console.error('Error listening to pending approvals:', err);
      setLoadingApprovals(false);
    });

    // Listen for all non-draft history orders
    const qAll = query(
      collection(db, 'replenishment_lists'),
      where('businessId', '==', business.id)
    );
    ...
    const unsubscribeAll = onSnapshot(qAll, (snapshot) => {
      const list: ReplenishmentList[] = [];
      snapshot.forEach(docSnap => {
        const d = { id: docSnap.id, ...docSnap.data() } as ReplenishmentList;
        if (d.status !== 'DRAFT') {
          list.push(d);
        }
      });
  ...
  useEffect(() => {
    const handleNavigateReplenishment = (e: Event) => {
      const customEvent = e as CustomEvent<{ tab?: 'SOLICITUD' | 'SUGGESTIONS' | 'APPROVALS' | 'HISTORY'; orderId?: string }>;
      ...
      if (detail?.orderId) {
        ...
        // If orderId is provided, find in pendingApprovals
        const foundPending = pendingApprovals.find(o => o.id === detail.orderId);
        if (foundPending && isAdmin) {
          setSelectedApprovalOrder(foundPending);
        }
      } else {
    ...
    window.addEventListener('minimarket:navigate-replenishment', handleNavigateReplenishment);
    return () => {
      window.removeEventListener('minimarket:navigate-replenishment', handleNavigateReplenishment);
    };
  }, [pendingApprovals, historyOrders, isAdmin]);

  // Active products in in-flight requests (Pending approval, Approved, Exported/Sent/Confirmed, Receiving pending/partial)
  const activeRequestedProductIds = useMemo(() => {
    const activeStatuses = new Set<ReplenishmentStatus>(['PENDING_APPROVAL', 'APPROVED', 'EXPORTED']);
    ...
    return set;
  }, [historyOrders, pendingApprovals]);
  ...
  // Admin-only estimated cost calculation
  const estimatedDraftCost = useMemo(() => {
    if (!isAdmin) return 0;
    return draftItems.reduce((sum, item) => {
      const p = productMap.get(item.productId);
      const cost = p?.costPrice || item.costPrice || 0;
      return sum + (cost * (Number(item.requestedQuantity) || 0));
    }, 0);
  }, [draftItems, isAdmin, productMap]);

  // Compute Auto Suggestions (products where stock < reorderPoint, not in draft, and not in active in-flight requests)
  const suggestions = useMemo(() => {
    return products.filter(p => {
      ...
      const notInActiveOrder = !activeRequestedProductIds.has(p.id);
      const notRejected = !rejectedSuggestionIds.has(p.id);
      return isBelowReorder && notInDraft && notInActiveOrder && notRejected;
    });
  }, [products, draftItems, activeRequestedProductIds, rejectedSuggestionIds]);
  ...
  const handleSubmitForApproval = async () => {
    ...
    try {
      ...
      await submitReplenishmentForApproval(
        savedDraft.id,
        business.id,
        userProfile.uid,
        userProfile.displayName || userProfile.email || 'Vendedor',
      ...
      setViewMode('OVERVIEW');
      setActiveTab('APPROVALS');
      showNotify('Solicitud enviada a aprobación de Administrador con éxito.', 'success');
    } catch (err: any) {
      console.error('Error submitting for approval:', err);
      showNotify(err?.message || 'Error al enviar la solicitud para aprobación.', 'error');
    } finally {
      setSubmittingApproval(false);
    }
  };

  // Toggle Admin approval required setting
  const handleToggleApprovalRequired = async () => {
    if (!business?.id || !isAdmin || !userProfile) return;
    setUpdatingApprovalSetting(true);
    try {
      const nextVal = !approvalRequired;
      await updateBusinessSettings(
        business.id,
        { replenishmentApprovalRequired: nextVal },
        { uid: userProfile.uid, email: userProfile.email || '' }
      );
      showNotify(
        nextVal
          ? 'Requerimiento de aprobación previa ACTIVADO.'
          : 'Requerimiento de aprobación previa DESACTIVADO.',
        'success'
      );
    } catch (err: any) {
      console.error('Error updating approval settings:', err);
      showNotify('Error al actualizar configuración de aprobación.', 'error');
    } finally {
      setUpdatingApprovalSetting(false);
    }
  };
  ...
  return (
    <div className="space-y-4 max-w-5xl mx-auto">
      {/* Toast Notification */}
      {notification && (
        <div className={`fixed bottom-5 right-5 z-[99999] flex items-center gap-2.5 px-4 py-3 rounded-2xl shadow-2xl border animate-in fade-in ${
        ...
        <div className="space-y-4">
          ...
          {wizardStep === 3 && (
            <div className="bg-white rounded-2xl border border-stone-200 p-4 sm:p-5 shadow-2xs space-y-4 max-w-2xl mx-auto">
              ...
              {/* High-level metrics */}
              <div className={`grid ${isAdmin ? 'grid-cols-3' : 'grid-cols-2'} gap-3 bg-blue-50/60 border border-blue-200 rounded-2xl p-4 text-center`}>
                <div>
                ...
                {isAdmin && (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-2 flex flex-col justify-center">
                    <span className="text-[10px] uppercase font-black text-emerald-900 block">Costo Est. (Admin)</span>
                    <span className="text-lg font-black text-emerald-700 font-mono">
                      ${estimatedDraftCost.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
              </div>
                          ...
                          <p className="text-[10px] text-stone-500 font-mono">
                            ...
                            {isAdmin && cost > 0 && (
                              <span className="ml-2 text-emerald-700 font-bold">
                                · Costo: ${cost.toFixed(2)} (Subtotal: ${(cost * item.requestedQuantity).toFixed(2)})
                              </span>
                            )}
                          </p>
                        </div>
                  ...
                  <>
                    {approvalRequired ? (
                      <button
                        ...
                        disabled={submittingApproval || draftItems.length === 0}
                        className="w-full py-3.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-extrabold text-sm rounded-2xl shadow-md flex items-center justify-center gap-2 cursor-pointer transition-colors"
                      >
                        <Send className="w-4 h-4" />
                        <span>{submittingApproval ? 'Solicitando Aprobación...' : 'SOLICITAR APROBACIÓN'}</span>
                      </button>
                    ) : (
                      canExport && (
                        <button
        ...
        <div className="space-y-4">
          ...
          {activeTab === 'APPROVALS' && (
            <div className="space-y-4">
              ...
              {isAdmin && (
                <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
                  <div className="flex items-start sm:items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-purple-950 text-sm flex items-center gap-2">
                        Control de Aprobaciones de Vendedores
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                          approvalRequired ? 'bg-purple-200 text-purple-900' : 'bg-stone-200 text-stone-700'
                        }`}>
                          {approvalRequired ? 'OBLIGATORIA' : 'OPCIONAL'}
                        </span>
                      </h4>
                      <p className="text-xs text-purple-800/80 mt-0.5">
                        {approvalRequired
                          ? 'Las solicitudes creadas por vendedores requieren revisión y aprobación de un Administrador antes de emitirse.'
                          : 'Los vendedores pueden generar pedidos directamente sin requerir aprobación previa.'}
                      </p>
                    </div>
                  </div>

                  <button
                    onClick={handleToggleApprovalRequired}
                    disabled={updatingApprovalSetting}
                    className={`px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all shadow-xs cursor-pointer shrink-0 ${
                      approvalRequired
                        ? 'bg-purple-700 hover:bg-purple-800 text-white'
                        : 'bg-stone-200 hover:bg-stone-300 text-stone-800'
                    }`}
                  >
                    {updatingApprovalSetting
                      ? 'Actualizando...'
                      : approvalRequired
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <h3 className="font-extrabold text-stone-900 text-sm uppercase tracking-wider flex items-center gap-2">
                    ...
                    Solicitudes Pendientes de Aprobación ({focusedOrderId ? pendingApprovals.filter(o => o.id === focusedOrderId).length : pendingApprovals.length})
                  </h3>
                </div>

                {loadingApprovals ? (
                  <div className="bg-white p-8 rounded-2xl border border-stone-200 text-center">
                    <RefreshCw className="w-6 h-6 animate-spin text-[#0057FF] mx-auto" />
                    <p className="text-xs text-stone-500 mt-2">Cargando solicitudes pendientes...</p>
                  </div>
                ) : (focusedOrderId ? pendingApprovals.filter(o => o.id === focusedOrderId) : pendingApprovals).length === 0 ? (
                  <div className="bg-stone-50 p-8 rounded-2xl border border-dashed border-stone-300 text-center space-y-2">
                    <CheckCircle2 className="w-8 h-8 text-emerald-500 mx-auto" />
                    <h4 className="font-bold text-stone-800 text-sm">No hay solicitudes pendientes</h4>
                    <p className="text-xs text-stone-500">Todas las solicitudes han sido revisadas o procesadas.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {(focusedOrderId ? pendingApprovals.filter(o => o.id === focusedOrderId) : pendingApprovals).map((order) => {
                      const estCost = isAdmin
                        ? (order.items || []).reduce((sum, it) => {
                            const p = productMap.get(it.productId);
                            const cost = p?.costPrice || it.costPrice || 0;
                            return sum + cost * (Number(it.requestedQuantity) || 0);
                          }, 0)
                        : 0;
                      ...
                      return (
                        <div
                          key={order.id}
                          className="bg-white border-2 border-purple-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow space-y-3"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-black bg-purple-100 text-purple-900 px-2 py-0.5 rounded-md font-mono">
                                  {formatRequestCode(order.id)}
                                </span>
                              ...
                              <p className="text-[11px] text-stone-500 mt-1">
                                Solicitado por: <strong>{order.submittedByName || order.createdByName || 'Vendedor'}</strong>
                              </p>
                              <p className="text-[10px] text-stone-400">
                                {order.submittedAt ? new Date(order.submittedAt).toLocaleString('es-AR') : 'Reciente'}
                              </p>
                            </div>

                            <span className="text-[11px] font-black bg-amber-100 text-amber-900 px-2.5 py-1 rounded-lg">
                              Pendiente
                            </span>
                          </div>

                          <div className="bg-stone-50 rounded-xl p-2.5 text-xs flex items-center justify-between text-stone-700 font-medium">
                            <span>{order.items?.length || 0} variedades</span>
                            <span className="font-mono font-bold text-[#0057FF]">{order.totalUnitsRequested || 0} unidades</span>
                            {isAdmin && estCost > 0 && (
                              <span className="font-mono font-black text-emerald-700">
                                Est: ${estCost.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                              </span>
                            )}
                          </div>

                          {order.notes && (
                            <p className="text-xs italic text-stone-500 bg-stone-50/50 p-2 rounded-lg border border-stone-100">
                              "{order.notes}"
                            </p>
                          )}
                          ...
                          <div className="pt-2 border-t border-stone-100 flex items-center justify-end gap-2">
                            {isAdmin ? (
                              <button
                                ...
                                className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs rounded-xl shadow-xs flex items-center justify-center gap-2 cursor-pointer transition-colors"
                              >
                                <CheckCheck className="w-4 h-4" />
                                <span>[ REVISAR Y APROBAR ]</span>
                              </button>
                            ) : (
                              <span className="text-xs text-stone-500 italic">Esperando revisión de Administrador</span>
                            )}
                          </div>
                        </div>
                      );
          ...
          {activeTab === 'HISTORY' && (
            <ReplenishmentHistoryView
              ...
              onSelectOrderForReview={(order) => setSelectedApprovalOrder(order)}
              onShareOrder={handleShareOrder}
              onShowNotify={showNotify}
              focusedOrderId={focusedOrderId}
              onClearFocusedOrder={() => setFocusedOrderId(null)}
            />
          )}
      ...
      {/* ADMIN APPROVAL / ADJUSTMENT MODAL */}
      {selectedApprovalOrder && (
        <ReplenishmentApprovalModal
          ...
          currentUserName={userProfile?.displayName || userProfile?.email || 'Administrador'}
          businessId={business?.id}
          businessName={business?.name}
          onClose={() => setSelectedApprovalOrder(null)}
          onApproved={(approvedOrder) => {
            setSelectedApprovalOrder(null);
            showNotify('Solicitud aprobada con éxito.', 'success');
          }}
          onRejected={(rejectedOrder) => {
            setSelectedApprovalOrder(null);
            showNotify('Solicitud rechazada.', 'info');
          }}
          onShowNotify={showNotify}
        />
      )}
