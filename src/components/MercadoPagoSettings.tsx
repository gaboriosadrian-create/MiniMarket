...
export const MercadoPagoSettings: React.FC<MercadoPagoSettingsProps> = ({
  ...
  const fetchPlatformConfig = async () => {
    try {
      const res = await fetch(getApiUrl('/api/mercadopago/platform-config'));
  ...
  const fetchMerchants = async () => {
    try {
      ...
      const res = await fetch(getApiUrl('/api/mercadopago/merchants'));
      const data = await res.json();
  ...
  // Load all businesses for Super Admin tenant switcher, or sync current business
  useEffect(() => {
    if (business?.id && !initialBusinessId && !isSuperAdmin) {
      setSelectedBusinessId(business.id);
    }
  }, [business?.id, initialBusinessId, isSuperAdmin]);
  ...
  const loadConfig = async (targetBId?: string) => {
    try {
      ...
      const res = await fetch(getApiUrl(`/api/mercadopago/config?businessId=${encodeURIComponent(bId)}`));
      const data = await res.json();
      ...
      const auditRes = await fetch(getApiUrl('/api/mercadopago/audits?limit=8'));
  ...
  const handleVerifyConnection = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl('/api/mercadopago/config/test'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          businessId: bId,
    ...
    <div className="space-y-5">
                        ...
                        <tr key={m.businessId} className="hover:bg-stone-50/60 transition-colors">
                          ...
                          <td className="py-3 px-3 text-right space-x-1.5">
                            <button
                              type="button"
                              onClick={() => {
                                const found = businesses.find((b) => b.id === m.businessId);
                                if (found) {
                                  setManagingBusiness(found);
                                } else {
                                  setManagingBusiness({
                                    id: m.businessId,
                                    name: businessName,
                                    status: 'active',
                                    createdAt: m.connectedAt || new Date().toISOString(),
                                  } as Business);
                                }
                              }}
                              className="px-2.5 py-1 bg-purple-100 hover:bg-purple-200 text-purple-900 text-[11px] font-bold rounded-lg transition-all cursor-pointer inline-flex items-center gap-1"
                              title="Gestionar Administradores y Datos del Comercio"
                            >
                              <Users className="w-3 h-3" />
                              <span>Admins & Datos</span>
                            </button>
      ...
      {managingBusiness && (
        <BusinessAdminsManagementModal
          ...
          superAdminUser={{
            uid: userProfile?.id || 'superadmin',
            email: userProfile?.email || 'superadmin@uwi.com',
          }}
          initialTab="admins"
