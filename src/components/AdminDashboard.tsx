...
import { CashControlModule } from './CashControlModule';
...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'pos' | 'control' | 'cash-control' | 'ventas' | 'caja' | 'compras' | 'obligations' | 'gastos' | 'analysis' | 'events' | 'products' | 'receivings' | 'adjustments' | 'replenishment' | 'sellers' | 'business' | 'config'
  >('pos');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (
        ...
        window.location.hash === '#personalizacion' ||
        window.location.hash === '#apariencia' ||
  ...
  useEffect(() => {
    setNavItems([
      ...
      },
      {
        ...
        isActive: activeTab === 'cash-control',
        onClick: () => setActiveTab('cash-control'),
      },
      {
        id: 'caja',
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-4 space-y-3">
      ...
      {/* POS / CAJA MODULE */}
      {activeTab === 'pos' && (
        <PosCaja onNavigateToCashControl={() => setActiveTab('cash-control')} />
      )}
      ...
      {/* CASH CONTROL MODULE (Sesiones, Turnos, Arqueos) */}
      {activeTab === 'cash-control' && (
        <CashControlModule />
      )}
