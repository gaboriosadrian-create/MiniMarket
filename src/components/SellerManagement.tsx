            ...
            return (
              ...
              >
                ...
                <div className="bg-stone-50/90 rounded-xl p-2.5 border border-stone-200/60 flex flex-wrap items-center gap-1.5 text-[11px] font-bold">
                  ...
                  {effPerms.cash.controlCaja && (
                    <span className="bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-lg flex items-center gap-1 border border-emerald-200">
                      <Calculator className="w-3 h-3 text-emerald-600" /> Control de Caja
                    </span>
                  )}
                </div>
              ...
              <div className="pt-2 border-t border-stone-100">
                ...
                <div className="bg-stone-50 p-3 rounded-2xl border border-stone-200 text-xs space-y-2">
                  ...
                  <label className="flex items-center space-x-2 cursor-pointer font-semibold text-stone-800">
                    <input
                      type="checkbox"
                      checked={newSellerPermissions.cash.controlCaja ?? false}
                      onChange={(e) =>
                        setNewSellerPermissions({
                          ...newSellerPermissions,
                          cash: { ...newSellerPermissions.cash, controlCaja: e.target.checked }
                        })
                      }
                      className="rounded text-blue-600 focus:ring-blue-500 w-4 h-4"
                    />
                    <span>Control de Caja (solo día actual)</span>
                  </label>
                </div>
              </div>
            ...
            <form onSubmit={handleSavePermissions} className="space-y-5">
              ...
              <div className="bg-stone-50/80 p-4 rounded-2xl border border-stone-200 space-y-2.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  ...
                  <label className="sm:col-span-2 flex items-start space-x-2.5 p-2.5 bg-emerald-50/50 rounded-xl border border-emerald-200/80 cursor-pointer hover:bg-emerald-50">
                    <input
                      type="checkbox"
                      checked={editPermissions.cash.controlCaja ?? false}
                      onChange={(e) =>
                        setEditPermissions({
                          ...editPermissions,
                          cash: { ...editPermissions.cash, controlCaja: e.target.checked }
                        })
                      }
                      className="rounded text-emerald-600 focus:ring-emerald-500 w-4 h-4 mt-0.5 cursor-pointer"
                    />
                    <div className="space-y-0.5">
                      <span className="font-bold text-stone-900 block">Control de Caja</span>
                      <p className="text-[11px] text-stone-600 font-normal leading-tight">
                        Permite al vendedor consultar el Control Diario correspondiente únicamente al día actual para verificar ventas, caja y cierre.
                      </p>
                    </div>
                  </label>
                </div>
